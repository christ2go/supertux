...
static PHYSFS_EnumerateCallbackResult add_to_dictionary_path(
...
static PHYSFS_EnumerateCallbackResult remove_from_dictionary_path(
    void* data, const char* origdir, const char* fname) {
  std::string full_path = FileSystem::join(origdir, fname);
  if (physfsutil::is_directory(full_path)) {
...
Addon& AddonManager::get_repository_addon(const AddonId& id) const {
  return get_addon(m_repository_addons, id, false);
}

Addon& AddonManager::get_installed_addon(const AddonId& id) const {
  return get_addon(m_installed_addons, id, true);
}

std::vector<AddonId> AddonManager::get_repository_addons() const {
  return get_addons(m_repository_addons);
}

std::vector<AddonId> AddonManager::get_installed_addons() const {
  return get_addons(m_installed_addons);
}

bool AddonManager::has_online_support() const { return true; }

bool AddonManager::has_been_updated() const { return m_has_been_updated; }

TransferStatusPtr AddonManager::request_check_online() {
...
TransferStatusPtr AddonManager::request_install_addon(const AddonId& addon_id) {
  if (m_transfer_status) {
    throw std::runtime_error(
...
void AddonManager::install_addon(const AddonId& addon_id) {
  {  // remove addon if it already exists
...
void AddonManager::uninstall_addon(const AddonId& addon_id) {
...
void AddonManager::enable_addon(const AddonId& addon_id) {
  ...
  if (addon.is_enabled()) {
    ...
    if (PHYSFS_mount(addon.get_install_filename().c_str(), mountpoint.c_str(),
      ...
      if (addon.get_type() == Addon::LANGUAGEPACK) {
        PHYSFS_enumerate(addon.get_id().c_str(), add_to_dictionary_path,
                         nullptr);
      }
...
void AddonManager::disable_addon(const AddonId& addon_id) {
...
bool AddonManager::is_old_enabled_addon(
    const std::unique_ptr<Addon>& addon) const {
  return addon->get_format() == Addon::ORIGINAL &&
         addon->get_type() != Addon::LANGUAGEPACK && addon->is_enabled();
}

bool AddonManager::is_old_addon_enabled() const {
  auto it = std::find_if(m_installed_addons.begin(), m_installed_addons.end(),
...
void AddonManager::disable_old_addons() {
...
void AddonManager::mount_old_addons() {
  std::string mountpoint;
  for (auto& addon : m_installed_addons) {
    if (is_old_enabled_addon(addon)) {
...
void AddonManager::unmount_old_addons() {
  for (auto& addon : m_installed_addons) {
    if (is_old_enabled_addon(addon)) {
      if (PHYSFS_unmount(addon->get_install_filename().c_str()) == 0) {
        log_warning << "Could not remove " << addon->get_install_filename()
...
bool AddonManager::is_from_old_addon(const std::string& filename) const {
...
bool AddonManager::is_addon_installed(const std::string& id) const {
  return std::any_of(
      get_installed_addons().begin(), get_installed_addons().end(),
      [id](const auto& installed_addon) { return installed_addon == id; });
}

std::vector<std::string> AddonManager::scan_for_archives() const {
...
std::string AddonManager::scan_for_info(
    const std::string& archive_os_path) const {
...
void AddonManager::add_installed_archive(const std::string& archive,
...
void AddonManager::add_installed_addons() {
...
AddonManager::AddonList AddonManager::parse_addon_infos(
...
void AddonManager::update() { m_downloader.update(); }

void AddonManager::check_for_langpack_updates() {
