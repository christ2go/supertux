 ...
 */
bool has_multibyte_mark(unsigned char c) { return ((c & 0300) == 0200); }
...
uint32_t decode_utf8(const std::string& text, size_t& p) {
  ...
  if ((c1 & 0200) == 0000) {
    ...
    if (!has_multibyte_mark(static_cast<unsigned char>(c2)))
      throw std::runtime_error("Malformed utf-8 sequence");
    p += 2;
    ...
    if (!has_multibyte_mark(static_cast<unsigned char>(c2)))
      throw std::runtime_error("Malformed utf-8 sequence");
    if (!has_multibyte_mark(static_cast<unsigned char>(c3)))
      throw std::runtime_error("Malformed utf-8 sequence");
    p += 3;
    ...
    if (!has_multibyte_mark(static_cast<unsigned char>(c3)))
      throw std::runtime_error("Malformed utf-8 sequence");
    if (!has_multibyte_mark(static_cast<unsigned char>(c4)))
      throw std::runtime_error("Malformed utf-8 sequence");
    p += 4;
...
bool UTF8Iterator::done() const { return pos > text.size(); }
