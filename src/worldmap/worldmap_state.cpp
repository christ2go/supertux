...
WorldMapState::WorldMapState(WorldMap& worldmap) : m_worldmap(worldmap) {}

void WorldMapState::load_state() {
  ...
  try {
    ...
    if (m_worldmap.m_map_filename != "/levels/world2/worldmap.stwm") {
      ...
      if (vm.has_property(old_map_filename.c_str())) {
        vm.rename_table_entry(old_map_filename.c_str(),
                              m_worldmap.m_map_filename.c_str());
      }
    }
          ...
          if (SQ_SUCCEEDED(sq_getstring(vm.get_vm(), -2, &key))) {
            ...
            if (tilemap != nullptr) {
              ...
              while (SQ_SUCCEEDED(sq_next(vm.get_vm(), -2))) {
                ...
                if (SQ_SUCCEEDED(
                        sq_getstring(vm.get_vm(), -2, &property_key))) {
                  auto propKey = std::string(property_key);
                  if (propKey == "alpha") {
                    float alpha_value = 1.0;
                    if (SQ_SUCCEEDED(
                            sq_getfloat(vm.get_vm(), -1, &alpha_value))) {
...
void WorldMapState::save_state() const {
  SquirrelVM& vm = SquirrelVirtualMachine::current()->get_vm();
  SQInteger oldtop = sq_gettop(vm.get_vm());
  ...
  try {
    ...
    vm.store_string("back",
                    direction_to_string(m_worldmap.m_tux->m_back_direction));
    ...
    for (auto& tilemap : m_worldmap.get_objects_by_type<::TileMap>()) {
      if (!tilemap.get_name().empty()) {
        ...
        if (SQ_FAILED(sq_createslot(vm.get_vm(), -3))) {
          throw std::runtime_error("failed to create '" + m_worldmap.m_name +
                                   "' table entry");
        }
      }
    }
