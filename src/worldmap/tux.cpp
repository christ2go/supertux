...
Tux::Tux(WorldMap* worldmap)
    : m_back_direction(),
      ...
      m_sprite(SpriteManager::current()->create(
          m_worldmap->get_savegame().get_player_status().worldmap_sprite)),
...
void Tux::draw(DrawingContext& context) {
  ...
  std::string action = get_action_prefix_for_bonus(
      m_worldmap->get_savegame().get_player_status().bonus);
  if (!action.empty()) {
...
std::string Tux::get_action_prefix_for_bonus(const BonusType& bonus) const {
...
void Tux::set_direction(Direction dir) { m_input_direction = dir; }

void Tux::set_ghost_mode(bool enabled) { m_ghost_mode = enabled; }

bool Tux::get_ghost_mode() const { return m_ghost_mode; }

void Tux::try_start_walking() {
  if (m_moving) return;
  if (m_input_direction == Direction::NONE) return;
...
bool Tux::can_walk(int tile_data, Direction dir) const {
  return m_ghost_mode ||
...
void Tux::change_sprite(SpriteChange* sprite_change) {
  ...
  if (sprite_change != nullptr) {
    ...
    m_worldmap->get_savegame().get_player_status().worldmap_sprite =
        sprite_change->m_sprite_name;
  }
}

void Tux::try_continue_walking(float dt_sec) {
  if (!m_moving) return;
  ...
  if (special_tile) {
    ...
    if ((m_direction == Direction::NORTH &&
         special_tile->get_apply_action_south()) ||
        (m_direction == Direction::SOUTH &&
         special_tile->get_apply_action_north()) ||
        (m_direction == Direction::WEST &&
         special_tile->get_apply_action_east()) ||
        (m_direction == Direction::EAST &&
         special_tile->get_apply_action_west())) {
  ...
  if ((m_worldmap->at_level()) ||
      ...
      (special_tile && !special_tile->is_passive_message() &&
       special_tile->get_script().empty()) ||
      (teleporter) || m_ghost_mode) {
    if (special_tile && !special_tile->get_map_message().empty() &&
  ...
  if ((m_direction != m_input_direction) &&
      can_walk(tile_data, m_input_direction)) {
    m_direction = m_input_direction;
    m_back_direction = reverse_dir(m_direction);
  } else {
    Direction dir = Direction::NONE;
    if (tile_data & Tile::WORLDMAP_NORTH &&
        m_back_direction != Direction::NORTH)
      dir = Direction::NORTH;
    else if (tile_data & Tile::WORLDMAP_SOUTH &&
             m_back_direction != Direction::SOUTH)
      dir = Direction::SOUTH;
    else if (tile_data & Tile::WORLDMAP_EAST &&
             m_back_direction != Direction::EAST)
      dir = Direction::EAST;
    else if (tile_data & Tile::WORLDMAP_WEST &&
             m_back_direction != Direction::WEST)
      dir = Direction::WEST;
  ...
  Vector next_tile;
  if (!m_ghost_mode &&
      !m_worldmap->path_ok(m_direction, m_tile_pos, &next_tile)) {
...
void Tux::update_input_direction() {
...
void Tux::process_special_tile(SpecialTile* special_tile) {
  if (!special_tile) {
    return;
  }

  if (special_tile->is_passive_message()) {
    m_worldmap->set_passive_message(special_tile->get_map_message(),
                                    map_message_TIME);
