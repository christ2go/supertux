...
ALuint SoundManager::load_file_into_buffer(SoundFile& file) {
  ...
  alBufferData(buffer, format, samples.get(), static_cast<ALsizei>(file.m_size),
...
std::unique_ptr<OpenALSoundSource> SoundManager::intern_create_sound_source(
...
std::unique_ptr<SoundSource> SoundManager::create_sound_source(
    const std::string& filename) {
  if (!m_sound_enabled) return create_dummy_sound_source();
...
void SoundManager::set_music_volume(int volume) {
  ...
  if (m_music_source != nullptr)
    m_music_source->set_volume(static_cast<float>(volume) / 100.0f);
}

void SoundManager::play_music(const std::string& filename, float fadetime) {
...
void SoundManager::pause_music(float fadetime) {
  if (m_music_source == nullptr) return;
