...
void StreamSoundSource::set_sound_file(std::unique_ptr<SoundFile> newfile) {
  m_file = std::move(newfile);

  ALint queued;
  alGetSourcei(m_source, AL_BUFFERS_QUEUED, &queued);
  for (size_t i = 0; i < STREAMFRAGMENTS - queued; ++i) {
    if (fillBufferAndQueue(m_buffers[i]) == false) break;
  }
}
...
void StreamSoundSource::update() {
  ...
  if (!playing()) {
    if (processed == 0 || !m_looping) return;
...
void StreamSoundSource::set_fading(FadeState state, float fade_time_) {
  m_fade_state = state;
...
bool StreamSoundSource::fillBufferAndQueue(ALuint buffer) {
  ...
  if (bytesread > 0) {
    ...
    try {
      alBufferData(buffer, format, bufferdata.get(),
                   static_cast<ALsizei>(bytesread), m_file->m_rate);
