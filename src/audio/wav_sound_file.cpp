...
WavSoundFile::WavSoundFile(PHYSFS_file* file_) : m_file(file_), m_datastart() {
  assert(m_file);
  char magic[4];
  if (PHYSFS_readBytes(m_file, magic, sizeof(magic)) <
  ...
  do {
    if (PHYSFS_readBytes(m_file, chunkmagic, sizeof(chunkmagic)) <
...
WavSoundFile::~WavSoundFile() { PHYSFS_close(m_file); }
...
size_t WavSoundFile::read(void* buffer, size_t buffer_size) {
