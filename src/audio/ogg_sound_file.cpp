...
OggSoundFile::OggSoundFile(PHYSFS_File* file_, double loop_begin_,
                           double loop_at_)
    : m_file(file_), m_vorbis_file(), m_loop_begin(), m_loop_at() {
...
OggSoundFile::~OggSoundFile() { ov_clear(&m_vorbis_file); }

size_t OggSoundFile::read(void* _buffer, size_t buffer_size) {
  ...
  while (buffer_size > 0) {
    ...
    long bytesRead =
        ov_read(&m_vorbis_file, buffer, static_cast<int>(bytes_to_read),
                bigendian, 2, 1, &section);
    if (bytesRead == 0) {
      break;
    }
...
void OggSoundFile::reset() { ov_pcm_seek(&m_vorbis_file, m_loop_begin); }

size_t OggSoundFile::cb_read(void* ptr, size_t size, size_t nmemb,
                             void* source) {
