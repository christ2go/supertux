...
void MarkerObject::draw(DrawingContext& context) {
  Vector dir = get_point_vector();
  if (dir.x == 0 && dir.y == 0) {
    context.color().draw_filled_rect(m_col.m_bbox, Color(1, 1, 1, 0.5), 7.5,
                                     LAYER_GUI - 20);
  } else {
    // draw a triangle
    dir = dir.unit() * 8;
    ...
    context.color().draw_triangle(pos + dir * 1.5, pos - dir + dir2,
                                  pos - dir - dir2, Color(1, 1, 1, 0.5),
                                  LAYER_GUI - 20);
  }
}
