...
void EditorLayersWidget::draw(DrawingContext& context) {
  if (m_object_tip) {
  ...
  context.color().draw_filled_rect(
      Rectf(Vector(0, static_cast<float>(m_Ypos)),
            Vector(static_cast<float>(m_Width),
  ...
  switch (m_hovered_item) {
    case HoveredItem::SPAWNPOINTS:
      target_rect = Rectf(Vector(0, static_cast<float>(m_Ypos)),
                          Vector(static_cast<float>(m_Xpos),
                                 static_cast<float>(SCREEN_HEIGHT)));
      break;

    case HoveredItem::SECTOR:
      target_rect =
          Rectf(Vector(static_cast<float>(m_Xpos), static_cast<float>(m_Ypos)),
                Vector(static_cast<float>(m_sector_text_width + m_Xpos),
                       static_cast<float>(SCREEN_HEIGHT)));
      break;
  ...
  if (draw_rect) {
    context.color().draw_filled_rect(target_rect, Color(0.9f, 0.9f, 1.0f, 0.6f),
                                     0.0f, LAYER_GUI - 5);
  }
...
void EditorLayersWidget::update(float dt_sec) {
...
bool EditorLayersWidget::on_mouse_button_up(
    const SDL_MouseButtonEvent& button) {
  return false;
}

bool EditorLayersWidget::on_mouse_button_down(
    const SDL_MouseButtonEvent& button) {
  if (button.button == SDL_BUTTON_LEFT) {
      ...
      case HoveredItem::LAYERS:
        if (m_hovered_layer >= m_layer_icons.size()) {
          ...
          if (m_layer_icons[m_hovered_layer]->is_tilemap()) {
            ...
            m_selected_tilemap = static_cast<TileMap*>(
                m_layer_icons[m_hovered_layer]->get_layer());
            m_selected_tilemap->m_editor_active = true;
            m_editor.edit_path(m_selected_tilemap->get_path(),
                               m_selected_tilemap);
          } else {
            auto cam = dynamic_cast<Camera*>(
                m_layer_icons[m_hovered_layer]->get_layer());
            if (cam) {
    ...
    if (m_hovered_item == HoveredItem::LAYERS &&
        m_hovered_layer < m_layer_icons.size()) {
      auto om = std::make_unique<ObjectMenu>(
          m_editor, m_layer_icons[m_hovered_layer]->get_layer());
...
bool EditorLayersWidget::on_mouse_motion(const SDL_MouseMotionEvent& motion) {
  Vector mouse_pos =
  ...
  if (x < 0) {
    ...
    if (x <= static_cast<float>(m_sector_text_width)) {
      ...
      if (m_hovered_layer != new_hovered_layer ||
...
void EditorLayersWidget::resize() {
  m_Ypos = SCREEN_HEIGHT - 32;
  m_Width = SCREEN_WIDTH - 128;
}

void EditorLayersWidget::setup() { resize(); }

void EditorLayersWidget::refresh() {
...
void EditorLayersWidget::refresh_sector_text() {
  m_sector_text = _("Sector") + ": " + m_editor.get_sector()->get_name();
  m_sector_text_width =
      int(Resources::normal_font->get_text_width(m_sector_text)) + 6;
}

void EditorLayersWidget::sort_layers() {
  std::sort(m_layer_icons.begin(), m_layer_icons.end(),
            [](const std::unique_ptr<LayerIcon>& lhs,
               const std::unique_ptr<LayerIcon>& rhs) {
              return lhs->get_zpos() < rhs->get_zpos();
            });
}

void EditorLayersWidget::add_layer(GameObject* layer) {
...
void EditorLayersWidget::update_tip() {
  if (m_hovered_layer >= m_layer_icons.size()) {
    m_object_tip = nullptr;
    return;
  }
  m_object_tip =
      std::make_unique<Tip>(*m_layer_icons[m_hovered_layer]->get_layer());
}

Vector EditorLayersWidget::get_layer_coords(const int pos) const {
  return Vector(static_cast<float>(pos * 35 + m_Xpos + m_sector_text_width),
                static_cast<float>(m_Ypos));
}

int EditorLayersWidget::get_layer_pos(const Vector& coords) const {
  return static_cast<int>((coords.x - static_cast<float>(m_Xpos) -
                           static_cast<float>(m_sector_text_width)) /
                          35.0f);
}
