...
void EditorOverlayWidget::update(float dt_sec) {
...
void EditorOverlayWidget::on_level_change() {
  m_dragged_object = nullptr;
  m_selected_object = nullptr;
  m_edited_path = nullptr;
  m_last_node_marker = nullptr;
...
void EditorOverlayWidget::delete_markers() {
...
Rectf EditorOverlayWidget::drag_rect() {
  ...
  return Rectf(static_cast<float>(start_x), static_cast<float>(start_y),
               static_cast<float>(end_x), static_cast<float>(end_y));
}

void EditorOverlayWidget::input_tile(const Vector& pos, uint32_t tile) {
...
void EditorOverlayWidget::put_tile() {
  auto tiles = m_editor.get_tiles();
  Vector add_tile;
  for (add_tile.x = static_cast<float>(tiles->m_width) - 1.0f;
       add_tile.x >= 0.0f; add_tile.x--) {
    for (add_tile.y = static_cast<float>(tiles->m_height) - 1.0f;
         add_tile.y >= 0; add_tile.y--) {
      input_tile(m_hovered_tile + add_tile,
...
void EditorOverlayWidget::draw_rectangle() {
  Rectf dr = drag_rect();
  ...
  for (int x = static_cast<int>(dr.get_left());
       x <= static_cast<int>(dr.get_right()); x++, x_++) {
    int y_ = sgn_y ? 0 : static_cast<int>(-dr.get_height());
    for (int y = static_cast<int>(dr.get_top());
         y <= static_cast<int>(dr.get_bottom()); y++, y_++) {
      input_tile(Vector(static_cast<float>(x), static_cast<float>(y)),
                 m_editor.get_tiles()->pos(x_, y_));
    }
  }
}

void EditorOverlayWidget::fill() {
  auto tiles = m_editor.get_tiles();
  auto tilemap = m_editor.get_selected_tilemap();
  if (!tilemap) {
    return;
  ...
  Uint32 replace_tile = tilemap->get_tile_id(
      static_cast<int>(m_hovered_tile.x), static_cast<int>(m_hovered_tile.y));
  ...
  while (pos_stack.size()) {
    ...
    input_tile(pos,
               tiles->pos(static_cast<int>(tpos.x), static_cast<int>(tpos.y)));
    Vector pos_;

    // Going left...
    pos_ = pos + Vector(-1, 0);
    if (pos_.x >= 0) {
      if (replace_tile == tilemap->get_tile_id(static_cast<int>(pos_.x),
                                               static_cast<int>(pos_.y)) &&
          replace_tile != tiles->pos(static_cast<int>(tpos.x - 1),
                                     static_cast<int>(tpos.y))) {
    ...
    if (pos_.y >= 0) {
      if (replace_tile == tilemap->get_tile_id(static_cast<int>(pos_.x),
                                               static_cast<int>(pos_.y)) &&
          replace_tile != tiles->pos(static_cast<int>(tpos.x),
                                     static_cast<int>(tpos.y - 1))) {
        pos_stack.push_back(pos_);
        continue;
      }
    }
...
void EditorOverlayWidget::hover_object() {
  for (auto& moving_object :
       m_editor.get_sector()->get_objects_by_type<MovingObject>()) {
...
void EditorOverlayWidget::edit_path(Path* path, GameObject* new_marked_object) {
  if (!path) return;
  delete_markers();
...
void EditorOverlayWidget::select_object() {
  delete_markers();
...
void EditorOverlayWidget::grab_object() {
  if (m_hovered_object) {
...
void EditorOverlayWidget::clone_object() {
  if (m_hovered_object && m_hovered_object->is_saveable()) {
    ...
    if (!pm) {
      ...
      auto game_object_uptr = [this] {
        ...
        return GameObjectFactory::instance().create(object_sx.get_name(),
                                                    object_sx.get_mapping());
      }();

      GameObject& game_object =
          m_editor.get_sector()->add_object(std::move(game_object_uptr));
...
void EditorOverlayWidget::show_object_menu(GameObject& object) {
...
void EditorOverlayWidget::move_object() {
  if (m_dragged_object) {
    ...
    if (snap_to_grid) {
      ...
      new_pos = (new_pos / static_cast<float>(snap_grid_size)).floor() *
                static_cast<float>(snap_grid_size);
...
void EditorOverlayWidget::rubber_object() {
  if (!m_edited_path) {
    delete_markers();
  }
  if (m_dragged_object) {
...
void EditorOverlayWidget::rubber_rect() {
  delete_markers();
  Rectf dr = drag_rect();
  for (auto& moving_object :
       m_editor.get_sector()->get_objects_by_type<MovingObject>()) {
...
void EditorOverlayWidget::update_node_iterators() {
...
void EditorOverlayWidget::add_path_node() {
  Path::Node new_node;
  new_node.position = m_sector_pos;
  new_node.time = 1;
  m_edited_path->m_nodes.insert(m_last_node_marker->m_node + 1, new_node);
  Sector::get().add<NodeMarker>(m_edited_path, m_edited_path->m_nodes.end() - 1,
                                m_edited_path->m_nodes.size() - 1);
...
void EditorOverlayWidget::put_object() {
  ...
  if (object_class[0] == '#') {
    ...
    if (snap_to_grid) {
      auto& snap_grid_size = snap_grid_sizes[selected_snap_grid_size];
      target_pos = (m_sector_pos / static_cast<float>(snap_grid_size)).floor() *
                   static_cast<float>(snap_grid_size);
    }
...
void EditorOverlayWidget::process_left_click() {
  m_dragging = true;
  m_dragging_right = false;
...
void EditorOverlayWidget::process_right_click() {
  switch (m_editor.get_tileselect_input_type()) {
    case EditorToolboxWidget::InputType::TILE:
      m_dragging = true;
...
Rectf EditorOverlayWidget::tile_drag_rect() {
...
Rectf EditorOverlayWidget::selection_draw_rect() {
...
void EditorOverlayWidget::update_tile_selection() {
  Rectf select = tile_drag_rect();
  auto tiles = m_editor.get_tiles();
  auto tilemap = m_editor.get_selected_tilemap();
  if (!tilemap) {
  ...
  for (int y = static_cast<int>(select.get_top());
       y < static_cast<int>(select.get_bottom()); y++) {
    for (int x = static_cast<int>(select.get_left());
         x < static_cast<int>(select.get_right()); x++) {
      if (x < 0 || y < 0 || x >= w || y >= h) {
...
bool EditorOverlayWidget::on_mouse_button_up(
    const SDL_MouseButtonEvent& button) {
  m_dragging = false;
  return true;
}

bool EditorOverlayWidget::on_mouse_button_down(
    const SDL_MouseButtonEvent& button) {
...
bool EditorOverlayWidget::on_mouse_motion(const SDL_MouseMotionEvent& motion) {
  m_mouse_pos =
      VideoSystem::current()->get_viewport().to_logical(motion.x, motion.y);
  update_pos();
...
bool EditorOverlayWidget::on_key_up(const SDL_KeyboardEvent& key) {
...
bool EditorOverlayWidget::on_key_down(const SDL_KeyboardEvent& key) {
  auto sym = key.keysym.sym;
  if (sym == SDLK_F8) {
...
void EditorOverlayWidget::update_pos() {
  if (m_editor.get_sector() == nullptr) return;

  m_sector_pos =
      m_mouse_pos + m_editor.get_sector()->get_camera().get_translation();
  m_hovered_tile = sp_to_tp(m_sector_pos);
  // update tip
  hover_object();
}

void EditorOverlayWidget::draw_tile_tip(DrawingContext& context) {
  if (m_editor.get_tileselect_input_type() ==
      EditorToolboxWidget::InputType::TILE) {
    auto tilemap = m_editor.get_selected_tilemap();
    if (!tilemap) {
      return;
    }
    ...
    for (drawn_tile.x = static_cast<float>(tiles->m_width) - 1.0f;
         drawn_tile.x >= 0.0f; drawn_tile.x--) {
      for (drawn_tile.y = static_cast<float>(tiles->m_height) - 1.0f;
           drawn_tile.y >= 0.0f; drawn_tile.y--) {
        Vector on_tile = m_hovered_tile + drawn_tile;

        if (m_editor.get_tiles()->empty() || on_tile.x < 0 || on_tile.y < 0 ||
            on_tile.x >= static_cast<float>(tilemap->get_width()) ||
            on_tile.y >= static_cast<float>(tilemap->get_height())) {
          continue;
        }
        uint32_t tile_id = tiles->pos(static_cast<int>(drawn_tile.x),
                                      static_cast<int>(drawn_tile.y));
        ...
        m_editor.get_sector()->camera->get_translation(), LAYER_GUI-11, Color(1,
        1, 1, 0.5));
        }*/
      }
    }
...
void EditorOverlayWidget::draw_tile_grid(DrawingContext& context,
  ...
  Rectf draw_rect = Rectf(
      cam_translation,
      cam_translation + Vector(static_cast<float>(context.get_width() - 128),
                               static_cast<float>(context.get_height() - 32)));
  Vector start =
      sp_to_tp(Vector(draw_rect.get_left(), draw_rect.get_top()), tile_size);
  Vector end = sp_to_tp(Vector(draw_rect.get_right(), draw_rect.get_bottom()),
                        tile_size);
  ...
  for (int i = static_cast<int>(start.x); i <= static_cast<int>(end.x); i++) {
    line_start =
        tile_screen_pos(Vector(static_cast<float>(i), 0.0f), tile_size);
    line_end = tile_screen_pos(Vector(static_cast<float>(i), end.y), tile_size);
    context.color().draw_line(line_start, line_end, line_color,
                              current_tm->get_layer());
  }

  for (int i = static_cast<int>(start.y); i <= static_cast<int>(end.y); i++) {
    line_start =
        tile_screen_pos(Vector(0.0f, static_cast<float>(i)), tile_size);
...
void EditorOverlayWidget::draw_tilemap_border(DrawingContext& context) {
  ...
  Vector end =
      tile_screen_pos(Vector(static_cast<float>(current_tm->get_width()),
                             static_cast<float>(current_tm->get_height())));
  context.color().draw_line(start, Vector(start.x, end.y), Color(1, 0, 1),
                            current_tm->get_layer());
  context.color().draw_line(start, Vector(end.x, start.y), Color(1, 0, 1),
                            current_tm->get_layer());
  context.color().draw_line(Vector(start.x, end.y), end, Color(1, 0, 1),
                            current_tm->get_layer());
  context.color().draw_line(Vector(end.x, start.y), end, Color(1, 0, 1),
                            current_tm->get_layer());
}

void EditorOverlayWidget::draw_path(DrawingContext& context) {
  if (!m_edited_path) return;
  if (!m_selected_object) return;
  if (!m_selected_object->is_valid()) return;
  if (!m_edited_path->is_valid()) return;

  for (auto i = m_edited_path->m_nodes.begin();
       i != m_edited_path->m_nodes.end(); ++i) {
    auto j = i + 1;
    Path::Node* node1 = &(*i);
    Path::Node* node2;
...
void EditorOverlayWidget::draw(DrawingContext& context) {
...
Vector EditorOverlayWidget::tp_to_sp(const Vector& tp, int tile_size) {
  auto tilemap = m_editor.get_selected_tilemap();
  if (!tilemap) {
    return Vector(0, 0);
  }
...
Vector EditorOverlayWidget::sp_to_tp(const Vector& sp, int tile_size) {
  auto tilemap = m_editor.get_selected_tilemap();
  if (!tilemap) {
    return Vector(0, 0);
  }
...
Vector EditorOverlayWidget::tile_screen_pos(const Vector& tp, int tile_size) {
