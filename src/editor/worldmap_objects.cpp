...
WorldmapObject::WorldmapObject(const ReaderMapping& mapping,
    ...
    : MovingSprite(mapping, default_sprite), m_tile_x(), m_tile_y() {
  m_col.m_bbox =
      Rectf(Vector(32 * m_col.m_bbox.get_left(), 32 * m_col.m_bbox.get_top()),
            Sizef(32.0f, 32.0f));
}

WorldmapObject::WorldmapObject(const ReaderMapping& mapping)
    : MovingSprite(mapping), m_tile_x(), m_tile_y() {
...
WorldmapObject::WorldmapObject(const Vector& pos,
                               const std::string& default_sprite)
    : MovingSprite(pos, default_sprite), m_tile_x(), m_tile_y() {
  m_col.m_bbox.set_left(32 * m_col.m_bbox.get_left());
  m_col.m_bbox.set_top(32 * m_col.m_bbox.get_top());
  m_col.m_bbox.set_size(32, 32);
}

ObjectSettings WorldmapObject::get_settings() {
...
void WorldmapObject::move_to(const Vector& pos) {
  Vector new_pos;
...
void LevelDot::after_editor_set() {}
...
ObjectSettings Teleporter::get_settings() {
  ObjectSettings result = WorldmapObject::get_settings();
...
ObjectSettings WorldmapSpawnPoint::get_settings() {
  ObjectSettings result = WorldmapObject::get_settings();

  result.add_worldmap_direction(_("Direction"), &m_dir,
                                worldmap::Direction::NONE, "auto-dir");
...
ObjectSettings SpriteChange::get_settings() {
  ObjectSettings result = WorldmapObject::get_settings();
  ...
  result.reorder({"change-on-touch", "initial-stay-action", "stay-group",
                  "sprite", "x", "y"});
...
ObjectSettings SpecialTile::get_settings() {
  ObjectSettings result = WorldmapObject::get_settings();
  ...
  result.add_text(_("Direction"), &m_apply_to_directions, "apply-to-direction",
                  std::string("north-east-south-west"));
  // result.add_sprite(_("Sprite"), &m_sprite_name, "sprite");

  result.reorder({"map-message", "invisible-tile", "script", "passive-message",
                  "apply-to-direction", "sprite", "x", "y"});
