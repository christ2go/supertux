...
bool Player::get_ghost_mode() const {
  SCRIPT_GUARD_DEFAULT;
...
void Player::do_scripting_controller(const std::string& control, bool pressed) {
  SCRIPT_GUARD_VOID;
  object.do_scripting_controller(control, pressed);
}

float Player::get_velocity_x() const {
  SCRIPT_GUARD_DEFAULT;
  return object.get_physic().get_velocity_x();
}

float Player::get_velocity_y() const {
  SCRIPT_GUARD_DEFAULT;
