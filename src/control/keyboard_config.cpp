...
KeyboardConfig::KeyboardConfig() : m_keymap(), m_jump_with_up_kbd(false) {
...
void KeyboardConfig::read(const ReaderMapping& keymap_mapping) {
  ...
  while (iter.next()) {
    ...
    if (maybe_control) {
      ...
      log_warning << "Invalid control '" << control_text << "' in keymap"
                  << std::endl;
    }
  }
}

void KeyboardConfig::bind_key(SDL_Keycode key, Control control) {
  ...
  if (i != m_keymap.end()) m_keymap.erase(i);

  // add new mapping
  m_keymap[key] = control;
}

SDL_Keycode KeyboardConfig::reversemap_key(Control c) const {
...
void KeyboardConfig::write(Writer& writer) {
