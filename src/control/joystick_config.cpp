...
int JoystickConfig::reversemap_joyaxis(Control c) const {
...
int JoystickConfig::reversemap_joybutton(Control c) const {
...
int JoystickConfig::reversemap_joyhat(Control c) const {
...
void JoystickConfig::print_joystick_mappings() const {
...
void JoystickConfig::unbind_joystick_control(Control control) {
  // remove all previous mappings for that control
  for (auto i = m_joy_axis_map.begin(); i != m_joy_axis_map.end();
       /* no ++i */) {
    if (i->second == control)
  ...
  for (auto i = m_joy_button_map.begin(); i != m_joy_button_map.end();
       /* no ++i */) {
    if (i->second == control)
...
void JoystickConfig::bind_joyaxis(JoystickID joy_id, int axis,
                                  Control control) {
...
void JoystickConfig::bind_joyhat(JoystickID joy_id, int dir, Control c) {
...
void JoystickConfig::bind_joybutton(JoystickID joy_id, int button,
                                    Control control) {
  unbind_joystick_control(control);
...
void JoystickConfig::read(const ReaderMapping& joystick_mapping) {
  ...
  while (iter.next()) {
    if (iter.get_key() == "map") {
      ...
      const boost::optional<Control> maybe_control =
          Control_from_string(control_text);
      if (!maybe_control) {
        log_info << "Invalid control '" << control_text << "' in buttonmap"
                 << std::endl;
      } else {
...
void JoystickConfig::write(Writer& writer) {
