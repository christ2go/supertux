...
Sprite::Sprite(SpriteData& newdata)
  ...
  if (!m_action) m_action = m_data.actions.begin()->second.get();
...
SpritePtr Sprite::clone() const { return SpritePtr(new Sprite(*this)); }

void Sprite::set_action(const std::string& name, int loops) {
  if (m_action && m_action->name == name) return;
...
bool Sprite::animation_done() const { return m_animation_loops == 0; }
...
float Sprite::get_current_hitbox_x_offset() const { return m_action->x_offset; }

float Sprite::get_current_hitbox_y_offset() const { return m_action->y_offset; }
...
Rectf Sprite::get_current_hitbox() const {
  return Rectf(m_action->x_offset, m_action->y_offset,
               m_action->x_offset + m_action->hitbox_w,
               m_action->y_offset + m_action->hitbox_h);
}

void Sprite::set_angle(float a) { m_angle = a; }

float Sprite::get_angle() const { return m_angle; }

void Sprite::set_color(const Color& c) { m_color = c; }

Color Sprite::get_color() const { return m_color; }

void Sprite::set_blend(const Blend& b) { m_blend = b; }

Blend Sprite::get_blend() const { return m_blend; }
