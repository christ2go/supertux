...
/** idle stages */
const std::string IDLE_STAGES[] = {"stand", "idle", "stand", "idle", "stand"};
...
const float JUMP_GRACE_TIME = 0.25f; /**< time before hitting the ground that
                                        the jump button may be pressed (and
                                        still trigger a jump) */
...
Player::Player(PlayerStatus& player_status, const std::string& name_)
    : ExposedObject<Player, scripting::Player>(this),
      ...
      m_lightsprite(SpriteManager::current()->create(
          "images/creatures/tux/light.sprite")),
      m_powersprite(SpriteManager::current()->create(
...
float Player::get_speedlimit() const { return m_speedlimit; }

void Player::set_speedlimit(float newlimit) { m_speedlimit = newlimit; }
...
void Player::use_scripting_controller(bool use_or_release) {
  if ((use_or_release == true) &&
      (m_controller != m_scripting_controller.get())) {
    m_scripting_controller_old = &get_controller();
    set_controller(m_scripting_controller.get());
  }
  if ((use_or_release == false) &&
      (m_controller == m_scripting_controller.get())) {
...
void Player::do_scripting_controller(const std::string& control_text,
                                     bool pressed) {
...
void Player::update(float dt_sec) {
  ...
  if (!m_dying && !m_deactivated) handle_input();
  ...
  */
  ...
  if (m_backflipping && !m_dying) {
    ...
    if (m_backflip_timer.started())
      m_physic.set_velocity_x(100.0f *
                              static_cast<float>(m_backflip_direction));
    // rotate sprite during flip
    m_sprite->set_angle(m_sprite->get_angle() +
                        (m_dir == Direction::LEFT ? 1 : -1) * dt_sec *
                            (360.0f / 0.5f));
  ...
  if (on_ground()) {
    ...
    if (m_backflipping && (m_backflip_timer.get_timegone() > 0.15f)) {
      ...
      // ourselves
      if (m_deactivated) do_standup(false);
    }
    if (m_player_status.bonus == AIR_BONUS)
      m_ability_time = static_cast<float>(m_player_status.max_air_time) *
                       GLIDE_TIME_PER_FLOWER;
  ...
  if (!m_ice_this_frame && on_ground()) m_on_ice = false;
...
bool Player::is_big() const {
  if (m_player_status.bonus == NO_BONUS) return false;
...
void Player::apply_friction() {
  if ((on_ground()) && (fabsf(m_physic.get_velocity_x()) < WALK_SPEED)) {
    ...
    float friction =
        WALK_ACCELERATION_X *
        (m_on_ice ? ICE_FRICTION_MULTIPLIER : NORMAL_FRICTION_MULTIPLIER);
...
void Player::handle_horizontal_input() {
  ...
  if (m_grabbed_object && m_grabbed_object->is_hampering()) {
    ...
    if (vx >= MAX_RUN_XM + BONUS_RUN_XM *
                               ((m_player_status.bonus == AIR_BONUS) ? 1 : 0) &&
        dirsign > 0) {
      vx = MAX_RUN_XM +
           BONUS_RUN_XM * ((m_player_status.bonus == AIR_BONUS) ? 1 : 0);
      ax = 0;
    ...
    if (on_ground()) {
      ...
      if (fabsf(vx) > SKID_XM && !m_skidding_timer.started()) {
        ...
        Sector::get().add<Particles>(
            Vector(m_dir == Direction::LEFT ? m_col.m_bbox.get_right()
                                            : m_col.m_bbox.get_left(),
                   m_col.m_bbox.get_bottom()),
            m_dir == Direction::LEFT ? 50 : -70,
            m_dir == Direction::LEFT ? 70 : -50, 260, 280, Vector(0, 300), 3,
...
void Player::do_duck() {
  ...
  if (m_physic.get_velocity_y() != 0) return;
  if (!on_ground()) return;
  if (m_does_buttjump) return;
...
void Player::handle_vertical_input() {
  ...
  if (m_controller->hold(Control::JUMP) && m_jump_button_timer.started() &&
    ...
    if (m_duck) {
      ...
      if (m_player_status.bonus == AIR_BONUS)
        do_jump((fabsf(m_physic.get_velocity_x()) > MAX_WALK_XM) ? -620.0f
                                                                 : -580.0f);
      else
        do_jump((fabsf(m_physic.get_velocity_x()) > MAX_WALK_XM) ? -580.0f
                                                                 : -520.0f);
    }
    // airflower glide only when holding jump key
  } else if (m_controller->hold(Control::JUMP) &&
             m_player_status.bonus == AIR_BONUS &&
             m_physic.get_velocity_y() > MAX_GLIDE_YM) {
    if (m_ability_time > 0 && !m_ability_timer.started())
      ...
      if ((m_ability_timer.get_timeleft() <= 0.05f) ||
  ...
  if (m_controller->hold(Control::DOWN) && !m_duck && is_big() &&
      !on_ground()) {
...
void Player::handle_input() {
  ...
  /* Peeking */
  if (m_controller->released(Control::PEEK_LEFT) ||
      m_controller->released(Control::PEEK_RIGHT)) {
    m_peekingX = Direction::AUTO;
  }
  if (m_controller->released(Control::PEEK_UP) ||
      m_controller->released(Control::PEEK_DOWN)) {
    m_peekingY = Direction::AUTO;
  }
  ...
  if (m_controller->pressed(Control::ACTION) &&
      (m_player_status.bonus == FIRE_BONUS ||
    ...
    if ((m_player_status.bonus == FIRE_BONUS &&
      ...
      Vector pos =
          get_pos() + ((m_dir == Direction::LEFT)
                           ? Vector(0, m_col.m_bbox.get_height() / 2)
                           : Vector(32, m_col.m_bbox.get_height() / 2));
      Sector::get().add<Bullet>(pos, m_physic.get_velocity_x(), m_dir,
                                m_player_status.bonus);
  ...
  if (m_controller->pressed(Control::DOWN) &&
      m_player_status.bonus == EARTH_BONUS && !m_cooldown_timer.started() &&
      on_ground()) {
    if (m_controller->hold(Control::ACTION) && !m_ability_timer.started()) {
      m_ability_timer.start(static_cast<float>(m_player_status.max_earth_time) *
  ...
  if (m_stone) apply_friction();

  /* Revert from Stone */
  if (m_stone && (!m_controller->hold(Control::ACTION) ||
                  m_ability_timer.get_timeleft() <= 0.5f)) {
    m_cooldown_timer.start(m_ability_timer.get_timegone() /
    ...
    for (int i = 0; i < 8; i++) {
      Vector ppos =
          Vector(m_col.m_bbox.get_left() + 8.0f +
                     16.0f * static_cast<float>(static_cast<int>(i / 4)),
                 m_col.m_bbox.get_top() + 16.0f * static_cast<float>(i % 4));
      float grey = graphicsRandom.randf(.4f, .8f);
      Color pcolor = Color(grey, grey, grey);
      Sector::get().add<Particles>(
          ppos, -60, 240, 42, 81, Vector(0.0f, 500.0f), 8, pcolor,
          4 + graphicsRandom.randf(-0.4f, 0.4f),
  ...
  if (!m_controller->hold(Control::ACTION) && m_grabbed_object) {
    ...
    if (moving_object) {
      ...
      Rectf dest_;
      dest_.set_bottom(m_col.m_bbox.get_top() +
                       m_col.m_bbox.get_height() * 0.66666f);
  ...
  if (m_backflipping &&
      (!m_controller->hold(Control::JUMP) && !m_backflip_timer.started())) {
    stop_backflipping();
  }
}
...
void Player::position_grabbed_object() {
  ...
  if (m_dir == Direction::LEFT) pos.x -= object_bbox.get_width();
...
void Player::try_grab() {
  if (m_controller->hold(Control::ACTION) && !m_grabbed_object && !m_duck) {
    ...
    if (m_dir == Direction::LEFT) {
      ...
      pos =
          Vector(m_col.m_bbox.get_right() + 5, m_col.m_bbox.get_bottom() - 16);
    }

    for (auto& moving_object :
         Sector::get().get_objects_by_type<MovingObject>()) {
...
void Player::handle_input_ghost() {
  ...
  if ((m_controller->hold(Control::UP)) ||
      (m_controller->hold(Control::JUMP))) {
    vy -= MAX_RUN_XM * 2;
  }
  if (m_controller->hold(Control::DOWN)) {
    vy += MAX_RUN_XM * 2;
...
void Player::add_coins(int count) { m_player_status.add_coins(count); }

int Player::get_coins() const { return m_player_status.coins; }

BonusType Player::string_to_bonus(const std::string& bonus) const {
...
bool Player::add_bonus(BonusType type, bool animate) {
  ...
  if (type == GROWUP_BONUS) {
    if (m_player_status.bonus != NO_BONUS) return true;
  }
...
bool Player::set_bonus(BonusType type, bool animate) {
  ...
  if ((m_player_status.bonus == NO_BONUS) && (type != NO_BONUS)) {
    ...
    if (animate) {
      m_growing = true;
      m_sprite->set_action(
          (m_dir == Direction::LEFT) ? "grow-left" : "grow-right", 1);
    }
    if (m_climbing) stop_climbing(*m_climbing);
  }
  ...
  if ((type == NO_BONUS) || (type == GROWUP_BONUS)) {
    Vector ppos =
        Vector((m_col.m_bbox.get_left() + m_col.m_bbox.get_right()) / 2,
               m_col.m_bbox.get_top());
    Vector pspeed =
        Vector(((m_dir == Direction::LEFT) ? 100.0f : -100.0f), -300.0f);
  ...
  if (!m_second_growup_sound_timer.started() && type > GROWUP_BONUS &&
      type != m_player_status.bonus) {
...
void Player::set_visible(bool visible_) { m_visible = visible_; }

bool Player::get_visible() const { return m_visible; }

void Player::kick() { m_kick_timer.start(KICK_TIME); }
...
void Player::draw(DrawingContext& context) {
  ...
  if (m_col.m_bbox.get_bottom() - 16 <
      Sector::get().get_camera().get_translation().y) {
    float px = m_col.m_bbox.get_left() +
               (m_col.m_bbox.get_right() - m_col.m_bbox.get_left() -
                static_cast<float>(m_airarrow.get()->get_width())) /
                   2.0f;
  ...
  if (m_dying) {
    ...
    if (fabsf(m_physic.get_velocity_x()) < 1.0f) {
      ...
      if (m_sprite->get_action().find("-stand-") == std::string::npos &&
          m_sprite->get_action().find("-idle-") == std::string::npos) {
        m_idle_stage = 0;
        m_idle_timer.start(static_cast<float>(IDLE_TIME[m_idle_stage]) /
                           1000.0f);

        m_sprite->set_action_continued(
            sa_prefix + ("-" + IDLE_STAGES[m_idle_stage]) + sa_postfix);
      } else if (m_idle_timer.check() ||
                 (IDLE_TIME[m_idle_stage] == 0 && m_sprite->animation_done())) {
        m_idle_stage++;
        if (m_idle_stage >= IDLE_STAGE_COUNT) m_idle_stage = 1;
        ...
        if (IDLE_TIME[m_idle_stage] == 0)
          m_sprite->set_action(
              sa_prefix + ("-" + IDLE_STAGES[m_idle_stage]) + sa_postfix, 1);
        else
          m_sprite->set_action(sa_prefix + ("-" + IDLE_STAGES[m_idle_stage]) +
                               sa_postfix);
      } else {
        m_sprite->set_action_continued(
            sa_prefix + ("-" + IDLE_STAGES[m_idle_stage]) + sa_postfix);
  ...
  */
  ...
  if (m_safe_timer.started() && size_t(g_game_time * 40) % 2)
    ;  // don't draw Tux
  else if (m_player_status.bonus ==
           EARTH_BONUS) {  // draw special effects with earthflower bonus
    // shake at end of maximum stone duration
    Vector shake_delta =
        (m_stone && m_ability_timer.get_timeleft() < 1.0f)
            ? Vector(graphicsRandom.randf(-3.0f, 3.0f) * 1.0f, 0)
            : Vector(0, 0);
    m_sprite->draw(context.color(), get_pos() + shake_delta, LAYER_OBJECTS + 1);
    // draw hardhat
    m_powersprite->draw(context.color(), get_pos() + shake_delta,
                        LAYER_OBJECTS + 1);
    // light
    ...
    if (m_cooldown_timer.started() && graphicsRandom.rand(0, 4) == 0) {
      float px = graphicsRandom.randf(m_col.m_bbox.get_left(),
    ...
    if (m_dying)
      m_sprite->draw(context.color(), get_pos(),
                     Sector::get().get_foremost_layer());
    else
...
void Player::collision_solid(const CollisionHit& hit) {
  if (hit.bottom) {
    ...
    if (m_does_buttjump) {
      ...
      m_on_ground_flag = false;
      Sector::get().add<Particles>(m_col.m_bbox.p2(), 50, 70, 260, 280,
                                   Vector(0, 300), 3, Color(.4f, .4f, .4f), 3,
                                   .8f, LAYER_OBJECTS + 1);
      Sector::get().add<Particles>(
          Vector(m_col.m_bbox.get_left(), m_col.m_bbox.get_bottom()), -70, -50,
          260, 280, Vector(0, 300), 3, Color(.4f, .4f, .4f), 3, .8f,
          LAYER_OBJECTS + 1);
      Sector::get().get_camera().shake(.1f, 0, 5);
    }

  } else if (hit.top) {
    if (m_physic.get_velocity_y() < 0) m_physic.set_velocity_y(.2f);
  }
...
void Player::kill(bool completely) {
  if (m_dying || m_deactivated || is_winning()) return;

  if (!completely &&
      (m_safe_timer.started() || m_invincible_timer.started() || m_stone))
    return;
  ...
  if (!completely && is_big()) {
    ...
    if (m_player_status.can_reach_checkpoint()) {
      for (int i = 0; i < 5; i++) {
        ...
        Sector::get().add<FallingCoin>(
            get_pos() + Vector(graphicsRandom.randf(5.0f),
                               graphicsRandom.randf(-32.0f, 18.0f)),
...
void Player::add_velocity(const Vector& velocity, const Vector& end_speed) {
  if (end_speed.x > 0)
    m_physic.set_velocity_x(
        std::min(m_physic.get_velocity_x() + velocity.x, end_speed.x));
  if (end_speed.x < 0)
    m_physic.set_velocity_x(
        std::max(m_physic.get_velocity_x() + velocity.x, end_speed.x));
  if (end_speed.y > 0)
    m_physic.set_velocity_y(
        std::min(m_physic.get_velocity_y() + velocity.y, end_speed.y));
  if (end_speed.y < 0)
    m_physic.set_velocity_y(
        std::max(m_physic.get_velocity_y() + velocity.y, end_speed.y));
}

Vector Player::get_velocity() const { return m_physic.get_velocity(); }

void Player::bounce(BadGuy&) {
  if (!(m_player_status.bonus == AIR_BONUS))
    m_physic.set_velocity_y(m_controller->hold(Control::JUMP) ? -520.0f
                                                              : -300.0f);
  else {
    m_physic.set_velocity_y(m_controller->hold(Control::JUMP) ? -580.0f
                                                              : -340.0f);
    m_ability_time = static_cast<float>(m_player_status.max_air_time) *
                     GLIDE_TIME_PER_FLOWER;
  }
...
void Player::walk(float speed) { m_physic.set_velocity_x(speed); }
...
void Player::set_edit_mode(bool enable) { m_edit_mode = enable; }
