...
WeakBlock::WeakBlock(const ReaderMapping& mapping)
    : MovingSprite(mapping, "images/objects/weak_block/strawbox.sprite",
...
HitResponse WeakBlock::collision_bullet(Bullet& bullet,
...
HitResponse WeakBlock::collision(GameObject& other, const CollisionHit& hit) {
  switch (state) {
    case STATE_NORMAL:
    ...
    case STATE_BURNING:
      if (m_sprite_name != "images/objects/weak_block/strawbox.sprite") break;
...
void WeakBlock::draw(DrawingContext& context) {
...
ObjectSettings WeakBlock::get_settings() {
