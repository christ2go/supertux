...
void IceCrusher::set_state(IceCrusherState state_, bool force) {
...
HitResponse IceCrusher::collision(GameObject& other, const CollisionHit& hit) {
...
void IceCrusher::collision_solid(const CollisionHit& hit) {
  switch (state) {
    case RECOVERING:
    case IDLE:
      break;
        ...
        if (ic_size == LARGE) {
          ...
          for (int j = 0; j < 9; j++) {
            Sector::get().add<Particles>(
                Vector(m_col.m_bbox.get_right() - static_cast<float>(j) * 8.0f -
                           4.0f,
                       m_col.m_bbox.get_bottom()),
                0, 90 - 5 * j, 140, 380, Vector(0.0f, 300.0f), 1,
                Color(.6f, .6f, .6f), 5, 1.8f, LAYER_OBJECTS + 1);
            Sector::get().add<Particles>(
                Vector(m_col.m_bbox.get_left() + static_cast<float>(j) * 8.0f +
                           4.0f,
                       m_col.m_bbox.get_bottom()),
                270 + 5 * j, 360, 140, 380, Vector(0.0f, 300.0f), 1,
                Color(.6f, .6f, .6f), 5, 1.8f, LAYER_OBJECTS + 1);
          }
        } else {
          ...
          for (int j = 0; j < 5; j++) {
            Sector::get().add<Particles>(
                Vector(m_col.m_bbox.get_right() - static_cast<float>(j) * 8.0f -
                           4.0f,
                       m_col.m_bbox.get_bottom()),
                0, 90 + 10 * j, 140, 260, Vector(0, 300), 1,
                Color(.6f, .6f, .6f), 4, 1.6f, LAYER_OBJECTS + 1);
            Sector::get().add<Particles>(
                Vector(m_col.m_bbox.get_left() + static_cast<float>(j) * 8.0f +
                           4.0f,
                       m_col.m_bbox.get_bottom()),
                270 + 10 * j, 360, 140, 260, Vector(0, 300), 1,
                Color(.6f, .6f, .6f), 4, 1.6f, LAYER_OBJECTS + 1);
          }
        }
...
void IceCrusher::update(float dt_sec) {
...
void IceCrusher::draw(DrawingContext& context) {
  ...
  if (!(state == CRUSHING) && m_sprite->has_action("whites")) {
    ...
    lefteye->draw(context.color(), get_pos() + eye_position(false),
                  m_layer + 1);
    righteye->draw(context.color(), get_pos() + eye_position(true),
                   m_layer + 1);
...
void IceCrusher::after_editor_set() {
  MovingSprite::after_editor_set();
  after_sprite_set();
}

bool IceCrusher::found_victim() const {
  if (auto* player = Sector::get().get_nearest_player(m_col.m_bbox)) {
    const Rectf& player_bbox = player->get_bbox();
    Rectf crush_area =
        Rectf(m_col.m_bbox.get_left() + 1, m_col.m_bbox.get_bottom(),
              m_col.m_bbox.get_right() - 1,
              std::max(m_col.m_bbox.get_bottom(), player_bbox.get_top() - 1));
    if ((player_bbox.get_top() >=
         m_col.m_bbox.get_bottom()) /* player is below crusher */
        && (player_bbox.get_right() >
            (m_col.m_bbox.get_left() - DROP_ACTIVATION_DISTANCE)) &&
        (player_bbox.get_left() <
         (m_col.m_bbox.get_right() + DROP_ACTIVATION_DISTANCE)) &&
        (Sector::get().is_free_of_statics(
            crush_area, this,
            false)) /* and area to player is free of objects */) {
      return true;
    }
  }
...
Vector IceCrusher::eye_position(bool right) const {
  if (state == IDLE) {
    if (auto* player = Sector::get().get_nearest_player(m_col.m_bbox)) {
      // Icecrusher focuses on approximate position of player's head
      const float player_focus_x =
          (player->get_bbox().get_right() + player->get_bbox().get_left()) *
          0.5f;
      const float player_focus_y = player->get_bbox().get_bottom() * 0.25f +
                                   player->get_bbox().get_top() * 0.75f;
      ...
      const float displacement_mag =
          powf(powf(displacement_x, 2.0f) + powf(displacement_y, 2.0f), 0.5f);
      // Determine weighting for eye displacement along x given icecrusher eye
      // shape
      int weight_x = m_sprite->get_width() / 64 *
                     (((displacement_x > 0) == right) ? 1 : 4);
      int weight_y = m_sprite->get_width() / 64 * 2;

      return Vector(
          displacement_x / displacement_mag * static_cast<float>(weight_x),
          displacement_y / displacement_mag * static_cast<float>(weight_y) -
              static_cast<float>(weight_y));
    }
  } else if (state == RECOVERING) {
    // Eyes spin while icecrusher is recovering, giving a dazed impression
    return Vector(
        sinf(
            (right ? 1 : -1) *  // X motion of each eye is opposite of the other
            (get_pos().y / 13 -  // Phase factor due to y position
             (ic_size == NORMAL ? RECOVER_SPEED_NORMAL : RECOVER_SPEED_LARGE) +
             cooldown_timer * 13.0f)) *  // Phase factor due to cooldown timer
                static_cast<float>(m_sprite->get_width()) /
                64.0f * 2.0f -
            (right ? 1 : -1) *  // Amplitude dependent on size
                static_cast<float>(m_sprite->get_width()) / 64.0f *
                2.0f,  // Offset to keep eyes visible
        ...
        cosf((right ? 3.1415f : 0.0f) +  // Eyes spin out of phase of eachother
             ...
             cooldown_timer * 13.0f) *  // Phase factor due to cooldown timer
                static_cast<float>(m_sprite->get_width()) /
                64.0f * 2.0f -  // Amplitude dependent on size
            static_cast<float>(m_sprite->get_width()) / 64.0f *
                2.0f);  // Offset to keep eyes visible
  }
...
void IceCrusher::after_sprite_set() {
  float sprite_width = static_cast<float>(m_sprite->get_width());
  if (sprite_width >= 128.0f) ic_size = LARGE;
