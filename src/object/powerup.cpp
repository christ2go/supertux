...
PowerUp::PowerUp(const ReaderMapping& mapping)
    : MovingSprite(mapping, "images/powerups/egg/egg.sprite", LAYER_OBJECTS,
                   COLGROUP_MOVING),
      physic(),
      script(),
      no_physics(),
...
void PowerUp::initialize() {
  ...
  if (m_sprite_name == "images/powerups/egg/egg.sprite" ||
    ...
    lightsprite->set_color(Color(0.0f, 0.3f, 0.0f));
             ...
             m_sprite_name == "/images/powerups/star/star.sprite") {
    lightsprite->set_color(Color(0.4f, 0.4f, 0.4f));
  }
}

void PowerUp::collision_solid(const CollisionHit& hit) {
  if (hit.bottom) {
...
HitResponse PowerUp::collision(GameObject& other, const CollisionHit&) {
  Player* player = dynamic_cast<Player*>(&other);
  if (player == nullptr) return FORCE_MOVE;
  ...
  if (m_sprite_name == "images/powerups/egg/egg.sprite" ||
    ...
    player->make_invincible();
             ...
             m_sprite_name == "/images/powerups/1up/1up.sprite") {
...
void PowerUp::update(float dt_sec) {
  if (!no_physics) m_col.m_movement = physic.get_movement(dt_sec);
    ...
    if (auto* player = Sector::get().get_nearest_player(m_col.m_bbox)) {
      ...
      if (disp_x * disp_x + disp_y * disp_y <= 256 * 256) {
        if (graphicsRandom.rand(0, 2) == 0) {
          float px = graphicsRandom.randf(m_col.m_bbox.get_left() * 1.0f,
                                          m_col.m_bbox.get_right() * 1.0f);
          float py = graphicsRandom.randf(m_col.m_bbox.get_top() * 1.0f,
                                          m_col.m_bbox.get_bottom() * 1.0f);
...
ObjectSettings PowerUp::get_settings() {
