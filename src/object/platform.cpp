...
void Platform::finish_construction() {
  if (!get_path()) {
...
ObjectSettings Platform::get_settings() {
...
HitResponse Platform::collision(GameObject& other, const CollisionHit&) {
...
void Platform::update(float dt_sec) {
  if (!get_path()) return;
  ...
  if (m_automatic) {
    if (!m_player_contact && !get_walker()->is_running()) {
      ...
      if (auto* player = Sector::get().get_nearest_player(m_col.m_bbox)) {
        int nearest_node_id =
            get_path()->get_nearest_node_no(player->get_bbox().p2());
    ...
    if (m_player_contact && !m_last_player_contact &&
        !get_walker()->is_running()) {
...
void Platform::goto_node(int node_no) { get_walker()->goto_node(node_no); }

void Platform::start_moving() { get_walker()->start_moving(); }

void Platform::stop_moving() { get_walker()->stop_moving(); }

void Platform::move_to(const Vector& pos) {
