...
void MovingSprite::update(float) {}

std::string MovingSprite::get_sprite_name() const { return m_sprite_name; }

void MovingSprite::set_action(const std::string& action, int loops) {
  m_sprite->set_action(action, loops);
  m_col.set_size(m_sprite->get_current_hitbox_width(),
                 m_sprite->get_current_hitbox_height());
}

void MovingSprite::set_action_centered(const std::string& action, int loops) {
...
void MovingSprite::spawn_explosion_sprites(int count,
  ...
  for (int i = 0; i < count; i++) {
    ...
    Sector::get().add<SpriteParticle>(sprite_path, "default", ppos,
                                      ANCHOR_MIDDLE, pspeed, paccel,
                                      LAYER_OBJECTS - 1);
  }
}
