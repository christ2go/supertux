...
void Explosion::explode() {
  ...
  m_sprite->set_animation_loops(1);  // TODO: this is necessary because
                                     // set_action will not set "loops" when
                                     // "action" is the default action
  m_sprite->set_angle(graphicsRandom.randf(0, 360));  // a random rotation on
                                                      // the sprite to make
                                                      // explosions appear more
                                                      // random
  if (hurt)
  ...
  Sector::get().add<Particles>(m_col.m_bbox.get_middle(), -360, 360, 450, 900,
                               accel, num_particles, Color(.4f, .4f, .4f), 3,
                               .8f, LAYER_OBJECTS - 1);
...
HitResponse Explosion::collision(GameObject& other, const CollisionHit&) {
