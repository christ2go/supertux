...
BonusBlock::BonusBlock(const ReaderMapping& mapping)
  ...
  while (iter.next()) {
    ...
    if (token == "x" || token == "y" || token == "sprite") {
          ...
          if (!mapping.get("custom-contents", content_collection)) {
            ...
            if (!object_specs.empty()) {
              if (object_specs.size() > 1) {
                log_warning << "only one object allowed in bonusblock "
                               "'custom-contents', ignoring the rest"
                            << std::endl;
              }

              const ReaderObject& spec = object_specs[0];
              auto game_object = GameObjectFactory::instance().create(
                  spec.get_name(), spec.get_mapping());
      ...
      if (m_contents == Content::CUSTOM && !m_object) {
        ...
        auto game_object =
            GameObjectFactory::instance().create(token, object_mapping);
  ...
  if (!Editor::is_active()) {
    if (m_contents == Content::CUSTOM && !m_object) {
      throw std::runtime_error(
          "Need to specify content object for custom block");
    }
  }

  if (m_contents == Content::LIGHT) {
    SoundManager::current()->preload("sounds/switch.ogg");
    m_lightsprite = Surface::from_file(
        "/images/objects/lightmap_light/bonusblock_light.png");
  }
}

BonusBlock::Content BonusBlock::get_content_by_data(int tile_data) const {
  ...
  switch (tile_data) {
    ...
    case 8:
      return Content::CUSTOM;  // Trampoline
    case 9:
      return Content::CUSTOM;  // Rock
    case 10:
      return Content::RAIN;
    case 11:
      return Content::EXPLODE;
    case 12:
      return Content::CUSTOM;  // Red potion
    case 13:
...
ObjectSettings BonusBlock::get_settings() {
  ...
  result.add_enum(
      _("Content"), reinterpret_cast<int*>(&m_contents),
      {_("Coin"), _("Growth (fire flower)"), _("Growth (ice flower)"),
       _("Growth (air flower)"), _("Growth (earth flower)"), _("Star"),
       _("Tux doll"), _("Custom"), _("Script"), _("Light"), _("Trampoline"),
       _("Coin rain"), _("Coin explosion")},
      {"coin", "firegrow", "icegrow", "airgrow", "earthgrow", "star", "1up",
       "custom", "script", "light", "trampoline", "rain", "explode"},
...
void BonusBlock::hit(Player& player) { try_open(&player); }

HitResponse BonusBlock::collision(GameObject& other, const CollisionHit& hit_) {
  auto player = dynamic_cast<Player*>(&other);
  if (player) {
    if (player->m_does_buttjump) try_drop(player);
  }
  ...
  if (badguy) {
    ...
    if (badguy->can_break() && (badguy->get_bbox().get_bottom() >
...
void BonusBlock::try_open(Player* player) {
  ...
  if (m_sprite->get_action() == "empty") return;

  if (player == nullptr) player = &Sector::get().get_player();

  if (player == nullptr) return;

  Direction direction =
      (player->get_bbox().get_middle().x > m_col.m_bbox.get_middle().x)
          ? Direction::LEFT
          : Direction::RIGHT;
  ...
  switch (m_contents) {
    case Content::COIN: {
      ...
      if (m_hit_counter != 0) Sector::get().get_level().m_stats.m_coins++;
      break;
    }
    ...
    case Content::TRAMPOLINE: {
      Sector::get().add<SpecialRiser>(
          get_pos(), std::make_unique<Trampoline>(get_pos(), false), true);
      play_upgrade_sound = true;
      break;
    }
    case Content::RAIN: {
  ...
  start_bounce(player);
  if (m_hit_counter <= 0 ||
      m_contents == Content::LIGHT) {  // use 0 to allow infinite hits
...
void BonusBlock::try_drop(Player* player) {
  SoundManager::current()->play("sounds/brick.wav");
  if (m_sprite->get_action() == "empty") return;
  ...
  if (!Sector::get().is_free_of_statics(dest_, this, true) &&
      !(m_contents == Content::ONEUP)) {
    try_open(player);
    return;
  }
  ...
  switch (m_contents) {
    ...
    case Content::CUSTOM: {
      // NOTE: non-portable trampolines could be moved to Content::CUSTOM, but
...
void BonusBlock::raise_growup_bonus(Player* player, const BonusType& bonus,
                                    const Direction& dir) {
...
void BonusBlock::drop_growup_bonus(const std::string& bonus_sprite_name,
                                   bool& countdown) {
...
void BonusBlock::draw(DrawingContext& context) {
  // do the regular drawing first
  Block::draw(context);
  // then Draw the light if on.
  if (m_sprite->get_action() == "on") {
    Vector pos =
        get_pos() + (m_col.m_bbox.get_size().as_vector() -
                     Vector(static_cast<float>(m_lightsprite->get_width()),
                            static_cast<float>(m_lightsprite->get_height()))) /
                        2.0f;
    context.light().draw_surface(m_lightsprite, pos, 10);
  }
}

BonusBlock::Content BonusBlock::get_content_from_string(
...
std::string BonusBlock::contents_to_string(
    const BonusBlock::Content& content) const {
  switch (m_contents) {
    case Content::COIN:
      return "coin";
    case Content::FIREGROW:
...
void BonusBlock::preload_contents(int d) {
  switch (d) {
    case 6:  // Light
      SoundManager::current()->preload("sounds/switch.ogg");
      m_lightsprite = Surface::from_file(
    ...
    case 9:  // Rock
      m_object =
          std::make_unique<Rock>(get_pos(), "images/objects/rock/rock.sprite");
      break;

    case 12:  // Red potion
      m_object = std::make_unique<PowerUp>(
