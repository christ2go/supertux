...
Particles::Particles(const Vector& epicenter, int min_angle, int max_angle,
  ...
  for (int p = 0; p < number; p++) {
    ...
    float angle = math::radians(graphicsRandom.randf(
        static_cast<float>(min_angle), static_cast<float>(max_angle)));
...
Particles::Particles(const Vector& epicenter, int min_angle, int max_angle,
  ...
  for (int p = 0; p < number; p++) {
    ...
    float angle = (min_angle == max_angle)
                      ...
                      : math::radians(graphicsRandom.randf(
                            static_cast<float>(min_angle),
...
void Particles::update(float dt_sec) {
  ...
  for (auto i = particles.begin(); i != particles.end();) {
    ...
    if ((*i)->pos.x < camera.x ||
        (*i)->pos.x > static_cast<float>(SCREEN_WIDTH) + camera.x ||
        (*i)->pos.y < camera.y ||
        (*i)->pos.y > static_cast<float>(SCREEN_HEIGHT) + camera.y) {
      i = particles.erase(i);
    } else {
      ++i;
    }
...
void Particles::draw(DrawingContext& context) {
  // draw particles
  for (auto& particle : particles) {
    context.color().draw_filled_rect(Rectf(particle->pos, Sizef(size, size)),
                                     color, drawing_layer);
  }
}
