...
UnstableTile::UnstableTile(const ReaderMapping& mapping)
    : MovingSprite(mapping, "images/objects/unstable_tile/snow.sprite",
...
HitResponse UnstableTile::collision(GameObject& other, const CollisionHit&) {
  if (state == STATE_NORMAL) {
    Player* player = dynamic_cast<Player*>(&other);
    if (player != nullptr && player->get_bbox().get_bottom() <
...
void UnstableTile::update(float dt_sec) {
  switch (state) {
    case STATE_NORMAL:
      break;

    case STATE_SHAKE:
      if (m_sprite->animation_done()) dissolve();
      break;
