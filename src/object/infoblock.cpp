...
ObjectSettings InfoBlock::get_settings() {
...
void InfoBlock::hit(Player& player) {
  start_bounce(&player);
...
HitResponse InfoBlock::collision(GameObject& other, const CollisionHit& hit_) {
  auto player = dynamic_cast<Player*>(&other);
  if (player) {
    if (player->m_does_buttjump) InfoBlock::hit(*player);
  }
  return Block::collision(other, hit_);
}

Player* InfoBlock::get_nearest_player() const {
  return Sector::get().get_nearest_player(m_col.m_bbox);
}

void InfoBlock::update(float dt_sec) {
  Block::update(dt_sec);
  ...
  if (m_shown_pct != m_dest_pct) {
    if (m_dest_pct > m_shown_pct)
      m_shown_pct = std::min(m_shown_pct + 2 * dt_sec, m_dest_pct);
    if (m_dest_pct < m_shown_pct)
      m_shown_pct = std::max(m_shown_pct - 2 * dt_sec, m_dest_pct);
  }
}

void InfoBlock::draw(DrawingContext& context) {
  Block::draw(context);
  ...
  float x1 =
      (m_col.m_bbox.get_left() + m_col.m_bbox.get_right()) / 2 - width / 2;
  float x2 =
      (m_col.m_bbox.get_left() + m_col.m_bbox.get_right()) / 2 + width / 2;
  ...
  context.color().draw_filled_rect(
      Rectf(Vector(x1 - border, y1 - border),
...
void InfoBlock::show_message() { m_dest_pct = 1; }

void InfoBlock::hide_message() { m_dest_pct = 0; }
