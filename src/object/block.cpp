...
HitResponse Block::collision(GameObject& other, const CollisionHit&) {
  ...
  bool is_bomb = (bomb != nullptr);  // bombs need to explode, although they are
                                     // considered portable
  bool hit_mo_from_below =
      ((moving_object == nullptr) || (moving_object->get_bbox().get_bottom() <
...
void Block::update(float dt_sec) {
  if (!m_bouncing) return;
...
void Block::draw(DrawingContext& context) {
  m_sprite->draw(context.color(), get_pos(), LAYER_OBJECTS + 1);
}

void Block::start_bounce(GameObject* hitter) {
  if (m_original_y == -1) {
  ...
  if (hitter_mo) {
    ...
    float offset = (m_col.m_bbox.get_middle().x - center_of_hitter) * 2 /
                   m_col.m_bbox.get_width();
    m_sprite->set_angle(BUMP_ROTATION_ANGLE * offset);
  }
}

void Block::start_break(GameObject* hitter) {
  start_bounce(hitter);
  m_breaking = true;
}
...
void Block::break_me() {
  ...
  for (const char* action :
    ...
    Sector::get().add<SpriteParticle>(m_sprite->clone(), action, pos,
                                      ANCHOR_MIDDLE, velocity,
                                      Vector(0, gravity), LAYER_OBJECTS + 1);
  }
