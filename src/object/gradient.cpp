...
ObjectSettings Gradient::get_settings() {
  ...
  result.add_enum(
      _("Direction"), reinterpret_cast<int*>(&m_gradient_direction),
      {_("Vertical"), _("Horizontal"), _("Vertical (whole sector)"),
       _("Horizontal (whole sector)")},
  ...
  result.add_enum(_("Blend mode"), reinterpret_cast<int*>(&m_blend),
                  ...
                  {"blend", "add", "mod", "none"},
                  static_cast<int>(Blend::BLEND), "blend");
...
void Gradient::update(float) {}

void Gradient::set_gradient(Color top, Color bottom) {
  m_gradient_top = top;
  m_gradient_bottom = bottom;

  if (m_gradient_top.red > 1.0f || m_gradient_top.green > 1.0f ||
      m_gradient_top.blue > 1.0f || m_gradient_top.alpha > 1.0f) {
    log_warning << "top gradient color has values above 1.0" << std::endl;
  }

  if (m_gradient_bottom.red > 1.0f || m_gradient_bottom.green > 1.0f ||
      m_gradient_bottom.blue > 1.0f || m_gradient_bottom.alpha > 1.0f) {
    log_warning << "bottom gradient color has values above 1.0" << std::endl;
  }
}

void Gradient::set_direction(const GradientDirection& direction) {
  m_gradient_direction = direction;
}

void Gradient::draw(DrawingContext& context) {
  ...
  if (m_gradient_direction != HORIZONTAL && m_gradient_direction != VERTICAL) {
    ...
    gradient_region = Rectf(0, 0, static_cast<float>(context.get_width()),
                            static_cast<float>(context.get_height()));
  }

  context.push_transform();
  context.set_translation(Vector(0, 0));
  context.get_canvas(m_target).draw_gradient(m_gradient_top, m_gradient_bottom,
                                             m_layer, m_gradient_direction,
                                             gradient_region, m_blend);
  context.pop_transform();
}

bool Gradient::is_saveable() const {
