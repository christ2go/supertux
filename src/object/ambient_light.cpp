...
void AmbientLight::update(float dt_sec) {
  if (m_ambient_light_fading) {
    m_ambient_light_fade_accum += dt_sec;
    float percent_done =
        m_ambient_light_fade_accum / m_ambient_light_fade_duration * 1.0f;
    float r = (1.0f - percent_done) * m_source_ambient_light.red +
              percent_done * m_target_ambient_light.red;
    float g = (1.0f - percent_done) * m_source_ambient_light.green +
              percent_done * m_target_ambient_light.green;
    float b = (1.0f - percent_done) * m_source_ambient_light.blue +
              percent_done * m_target_ambient_light.blue;
...
void AmbientLight::draw(DrawingContext& context) {
  context.set_ambient_color(m_ambient_light);
}

void AmbientLight::set_ambient_light(const Color& ambient_light) {
  m_ambient_light = ambient_light;
}

Color AmbientLight::get_ambient_light() const { return m_ambient_light; }

void AmbientLight::fade_to_ambient_light(float red, float green, float blue,
                                         float seconds) {
  if (seconds == 0) {
...
ObjectSettings AmbientLight::get_settings() {
