...
void InvisibleBlock::draw(DrawingContext& context) {
  if (visible || Editor::is_active())
    m_sprite->draw(context.color(), get_pos(), LAYER_OBJECTS);
}

bool InvisibleBlock::collides(GameObject& other, const CollisionHit&) const {
...
void InvisibleBlock::hit(Player& player) {
