...
void TextObject::set_font(const std::string& name_) {
  if (name_ == "normal") {
    m_font = Resources::normal_font;
  } else if (name_ == "big") {
...
void TextObject::set_text(const std::string& text_) {
  m_text = text_;
  wrap_text();
}

void TextObject::fade_in(float fadetime_) {
  m_fadetime = fadetime_;
  m_fading = fadetime_;
}

void TextObject::fade_out(float fadetime_) {
  m_fadetime = fadetime_;
  m_fading = -fadetime_;
}

void TextObject::set_visible(bool visible_) {
  m_visible = visible_;
  m_fading = 0;
}

void TextObject::set_centered(bool centered_) { m_centered = centered_; }
...
void TextObject::draw(DrawingContext& context) {
  ...
  Vector spos =
      ...
      get_anchor_pos(
          Rectf(0, 0, static_cast<float>(context.get_width()),
                static_cast<float>(context.get_height() + SCREEN_HEIGHT) -
                    340.0f),
  ...
  context.color().draw_filled_rect(Rectf(spos, Sizef(width, height)),
                                   ...
                                   LAYER_GUI + 50);
  if (m_centered) {
    context.color().draw_center_text(m_font, m_wrapped_text, spos,
                                     LAYER_GUI + 60, TextObject::default_color);
  } else {
    context.color().draw_text(m_font, m_wrapped_text, spos + Vector(10, 10),
                              ALIGN_LEFT, LAYER_GUI + 60,
