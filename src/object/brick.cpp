...
HitResponse Brick::collision(GameObject& other, const CollisionHit& hit_) {
  ...
  if (icecrusher && m_coin_counter == 0) try_break(player);
  return Block::collision(other, hit_);
}

void Brick::try_break(Player* player) {
  if (m_sprite->get_action() == "empty") return;
  ...
  if (m_coin_counter > 0) {
    ...
    if (m_coin_counter == 0) m_sprite->set_action("empty");
    start_bounce(player);
  } else if (m_breakable) {
    if (player) {
      if (player->is_big()) {
...
ObjectSettings Brick::get_settings() {
