...
ObjectSettings SequenceTrigger::get_settings() {
  ...
  result.add_enum(_("Sequence"), reinterpret_cast<int*>(&sequence),
                  ...
                  {"endsequence", "stoptux", "fireworks"}, boost::none,
                  "sequence");

  result.add_text(_("New worldmap spawnpoint"), &new_spawnpoint,
                  "new_spawnpoint");
...
void SequenceTrigger::after_editor_set() {
  m_col.m_bbox.set_size(new_size.x, new_size.y);
}

void SequenceTrigger::event(Player& player, EventType type) {
...
std::string SequenceTrigger::get_sequence_name() const {
  return sequence_to_string(sequence);
}

void SequenceTrigger::draw(DrawingContext& context) {
  if (Editor::is_active() || g_debug.show_collision_rects) {
    context.color().draw_filled_rect(
        m_col.m_bbox, Color(1.0f, 0.0f, 0.0f, 0.6f), 0.0f, LAYER_OBJECTS);
  }
}
