...
SurfacePtr Surface::from_reader(const ReaderMapping& mapping,
  ...
  if (mapping.get("diffuse-texture", diffuse_texture_mapping)) {
    diffuse_texture =
        TextureManager::current()->get(*diffuse_texture_mapping, rect);
  }
  ...
  if (mapping.get("displacement-texture", displacement_texture_mapping)) {
    displacement_texture =
        TextureManager::current()->get(*displacement_texture_mapping, rect);
  }
...
SurfacePtr Surface::from_file(const std::string& filename,
                              const boost::optional<Rect>& rect) {
...
Surface::Surface(const TexturePtr& diffuse_texture,
    ...
    : m_diffuse_texture(diffuse_texture),
      ...
      m_region(0, 0, m_diffuse_texture->get_image_width(),
               m_diffuse_texture->get_image_height()),
      m_flip(flip) {}

Surface::Surface(const TexturePtr& diffuse_texture,
                 const TexturePtr& displacement_texture, const Rect& region,
                 Flip flip)
    : m_diffuse_texture(diffuse_texture),
      m_displacement_texture(displacement_texture),
      m_region(region),
...
SurfacePtr Surface::clone(Flip flip) const {
  SurfacePtr surface(new Surface(m_diffuse_texture, m_displacement_texture,
                                 m_region, m_flip ^ flip));
  return surface;
}

SurfacePtr Surface::region(const Rect& rect) const {
  SurfacePtr surface(
      new Surface(m_diffuse_texture, m_displacement_texture, rect, m_flip));
  return surface;
}

TexturePtr Surface::get_texture() const { return m_diffuse_texture; }

TexturePtr Surface::get_displacement_texture() const {
