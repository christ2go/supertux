...
SDLBaseVideoSystem::SDLBaseVideoSystem()
    : m_sdl_window(nullptr, &SDL_DestroyWindow), m_desktop_size() {
...
void SDLBaseVideoSystem::set_gamma(float gamma) {
  Uint16 ramp[256];
...
void SDLBaseVideoSystem::create_sdl_window(Uint32 flags) {
  ...
  m_sdl_window.reset(SDL_CreateWindow("SuperTux", SDL_WINDOWPOS_UNDEFINED,
...
void SDLBaseVideoSystem::apply_video_mode() {
  if (!g_config->use_fullscreen) {
    ...
    Size window_size;
    SDL_GetWindowSize(m_sdl_window.get(), &window_size.width,
                      &window_size.height);
    if (g_config->window_size.width != window_size.width ||
        g_config->window_size.height != window_size.height) {
      SDL_SetWindowSize(m_sdl_window.get(), g_config->window_size.width,
                        g_config->window_size.height);
    }

#if SDL_VERSION_ATLEAST(2, 0, 5)
    SDL_SetWindowResizable(m_sdl_window.get(),
                           static_cast<SDL_bool>(g_config->window_resizable));
#endif
  } else {
    if (g_config->fullscreen_size.width == 0 &&
        g_config->fullscreen_size.height == 0) {
      if (SDL_SetWindowFullscreen(m_sdl_window.get(),
