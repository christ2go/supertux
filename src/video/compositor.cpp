...
Compositor::Compositor(VideoSystem& video_system)
    : m_video_system(video_system), m_obst(), m_drawing_contexts() {
  obstack_init(&m_obst);
}
...
DrawingContext& Compositor::make_context(bool overlay) {
  m_drawing_contexts.emplace_back(
      new DrawingContext(m_video_system, m_obst, overlay));
...
void Compositor::render() {
  ...
  bool use_lightmap = std::any_of(
      m_drawing_contexts.begin(), m_drawing_contexts.end(),
      [](std::unique_ptr<DrawingContext>& ctx) { return ctx->use_lightmap(); });
  ...
  // compose the screen
  {
    ...
    if (use_lightmap) {
      ...
      if (texture) {
        ...
        request.srcrects.emplace_back(
            0, 0, static_cast<float>(texture->get_image_width()),
            static_cast<float>(texture->get_image_height()));
        request.dstrects.emplace_back(Vector(0, 0),
                                      lightmap.get_logical_size());
