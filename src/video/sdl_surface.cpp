...
SDLSurfacePtr SDLSurface::create_rgba(int width, int height) {
...
SDLSurfacePtr SDLSurface::create_rgb(int width, int height) {
#if SDL_BYTEORDER == SDL_BIG_ENDIAN
  Uint32 rmask = 0xff000000;
  Uint32 gmask = 0x00ff0000;
  Uint32 bmask = 0x0000ff00;
...
#endif
  SDLSurfacePtr surface(
      SDL_CreateRGBSurface(0, width, height, 24, rmask, gmask, bmask, amask));
  if (!surface) {
    std::ostringstream out;
    out << "failed to create SDL_Surface: " << SDL_GetError();
    throw std::runtime_error(out.str());
...
SDLSurfacePtr SDLSurface::from_file(const std::string& filename) {
...
int SDLSurface::save_png(const SDL_Surface& surface,
  ...
  if (ret < 0) {
    log_warning << "Saving " << filename << " failed: " << SDL_GetError()
                << std::endl;
    return false;
  } else {
    return true;
