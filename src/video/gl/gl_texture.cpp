...
GLTexture::GLTexture(int width, int height, boost::optional<Color> fill_color)
  ...
  try {
    ...
    if (fill_color) {
      std::vector<uint32_t> pixels(m_texture_width * m_texture_height,
                                   fill_color->rgba());
      glTexImage2D(GL_TEXTURE_2D, 0, static_cast<GLint>(GL_RGBA),
                   m_texture_width, m_texture_height, 0, GL_RGBA,
...
GLTexture::GLTexture(const SDL_Surface& image, const Sampler& sampler)
  ...
  SDLSurfacePtr convert =
      SDLSurface::create_rgba(m_texture_width, m_texture_height);

  SDL_SetSurfaceBlendMode(const_cast<SDL_Surface*>(&image), SDL_BLENDMODE_NONE);
  SDL_BlitSurface(const_cast<SDL_Surface*>(&image), nullptr, convert.get(),
                  nullptr);
  ...
  if (m_image_width != m_texture_width || m_image_height != m_texture_height) {
    ...
    if (m_image_width != m_texture_width) {
      ...
      for (int x = m_image_width; x < m_texture_width; ++x) {
        ...
        SDL_BlitSurface(const_cast<SDL_Surface*>(&image), &srcrect,
                        convert.get(), &dstrect);
      }
    }
    ...
    if (m_image_width != m_texture_width &&
        m_image_height != m_texture_height) {
      const int bpp = convert->format->BytesPerPixel;
      const int x = m_image_width - 1;
      const int y = m_image_height - 1;
      Uint32 color =
          *reinterpret_cast<Uint32*>(static_cast<uint8_t*>(convert->pixels) +
                                     y * convert->pitch + x * bpp);
      SDL_Rect dstrect{m_image_width, m_image_height, m_texture_width,
                       m_texture_height};
  ...
  try {
    ...
    assert(convert->pitch ==
           static_cast<int>(m_texture_width * convert->format->BytesPerPixel));
#endif
...
void GLTexture::set_texture_params() {
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                  static_cast<GLint>(m_sampler.get_filter()));
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER,
                  static_cast<GLint>(m_sampler.get_filter()));

  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
                  static_cast<GLint>(m_sampler.get_wrap_s()));
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T,
                  static_cast<GLint>(m_sampler.get_wrap_t()));
