...
inline std::tuple<GLenum, GLenum> blend_factor(Blend blend) {
  ...
  switch (blend) {
    case Blend::NONE:
      return B(GL_ONE, GL_ZERO);
    case Blend::BLEND:
      return B(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    case Blend::ADD:
      return B(GL_SRC_ALPHA, GL_ONE);
    case Blend::MOD:
      return B(GL_DST_COLOR, GL_ZERO);
    default:
      return B(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
  }
}

inline GLenum sfactor(Blend blend) { return std::get<0>(blend_factor(blend)); }

inline GLenum dfactor(Blend blend) { return std::get<1>(blend_factor(blend)); }

}  // namespace

GLPainter::GLPainter(GLVideoSystem& video_system, GLRenderer& renderer)
    : m_video_system(video_system), m_renderer(renderer) {}

void GLPainter::draw_texture(const TextureRequest& request) {
  assert_gl();
  ...
  for (size_t i = 0; i < request.srcrects.size(); ++i) {
    ...
    float uv_left = request.srcrects[i].get_left() /
                    static_cast<float>(texture.get_texture_width());
    float uv_top = request.srcrects[i].get_top() /
                   static_cast<float>(texture.get_texture_height());
    float uv_right = request.srcrects[i].get_right() /
                     static_cast<float>(texture.get_texture_width());
    float uv_bottom = request.srcrects[i].get_bottom() /
                      static_cast<float>(texture.get_texture_height());

    if (request.flip & HORIZONTAL_FLIP) std::swap(uv_left, uv_right);

    if (request.flip & VERTICAL_FLIP) std::swap(uv_top, uv_bottom);
    ...
    if (request.angles[i] == 0.0f) {
      ...
      vertices.insert(vertices.end(), std::begin(vertices_lst),
                      std::end(vertices_lst));

      auto uvs_lst = {
          uv_left, uv_top,    uv_right, uv_top, uv_right, uv_bottom,

          uv_left, uv_bottom, uv_left,  uv_top, uv_right, uv_bottom,
      };
      ...
      const float vertices_lst[] = {
          new_left * ca - new_top * sa + center_x,
          new_left * sa + new_top * ca + center_y,
          new_right * ca - new_top * sa + center_x,
          new_right * sa + new_top * ca + center_y,
          new_right * ca - new_bottom * sa + center_x,
          new_right * sa + new_bottom * ca + center_y,

          new_left * ca - new_bottom * sa + center_x,
          new_left * sa + new_bottom * ca + center_y,
          new_left * ca - new_top * sa + center_x,
          new_left * sa + new_top * ca + center_y,
          new_right * ca - new_bottom * sa + center_x,
          new_right * sa + new_bottom * ca + center_y,
  ...
  context.set_color(Color(request.color.red, request.color.green,
                          request.color.blue,
                          request.color.alpha * request.alpha));

  context.draw_arrays(GL_TRIANGLES, 0,
                      static_cast<GLsizei>(request.srcrects.size() * 2 * 3));

  assert_gl();
}

void GLPainter::draw_gradient(const GradientRequest& request) {
  assert_gl();
  ...
  const float vertices[] = {region.get_left(),  region.get_top(),
...
void GLPainter::draw_filled_rect(const FillRectRequest& request) {
  assert_gl();
  ...
  if (request.radius != 0.0f) {
    ...
    // just graphic junk
    const float radius =
        std::min(request.radius, std::min(request.rect.get_width() / 2.0f,
                                          request.rect.get_height() / 2.0f));

    // inner rectangle
    const Rectf irect(
        request.rect.get_left() + radius, request.rect.get_top() + radius,
        request.rect.get_right() - radius, request.rect.get_bottom() - radius);
    ...
    for (int i = 0; i <= n; ++i) {
      const float x =
          sinf(static_cast<float>(i) * math::PI_2 / static_cast<float>(n)) *
          radius;
      const float y =
          cosf(static_cast<float>(i) * math::PI_2 / static_cast<float>(n)) *
          radius;
    ...
    context.draw_arrays(GL_TRIANGLE_STRIP, 0,
...
void GLPainter::draw_inverse_ellipse(const InverseEllipseRequest& request) {
  assert_gl();
  ...
  for (int i = 0; i < slices; ++i) {
    const float ex1 =
        sinf(math::PI_2 / static_cast<float>(slices) * static_cast<float>(i)) *
        w;
    const float ey1 =
        cosf(math::PI_2 / static_cast<float>(slices) * static_cast<float>(i)) *
        h;

    const float ex2 = sinf(math::PI_2 / static_cast<float>(slices) *
                           static_cast<float>(i + 1)) *
                      w;
    const float ey2 = cosf(math::PI_2 / static_cast<float>(slices) *
                           static_cast<float>(i + 1)) *
                      h;
...
void GLPainter::draw_line(const LineRequest& request) {
  assert_gl();
  ...
  const float vertices[] = {
      (x1 - x_step), (y1 - y_step), (x2 - x_step), (y2 - y_step),
      (x1 + x_step), (y1 + y_step), (x2 + x_step), (y2 + y_step),
  };
...
void GLPainter::draw_triangle(const TriangleRequest& request) {
  assert_gl();

  const float vertices[] = {
      request.pos1.x, request.pos1.y, request.pos2.x,
      request.pos2.y, request.pos3.x, request.pos3.y,
  };
...
void GLPainter::get_pixel(const GetPixelRequest& request) const {
  assert_gl();

  const Rect& rect = m_renderer.get_rect();
  const Size& logical_size = m_renderer.get_logical_size();

  float x = request.pos.x * static_cast<float>(rect.get_width()) /
            static_cast<float>(logical_size.width);
  float y = request.pos.y * static_cast<float>(rect.get_height()) /
  ...
  *(request.color_ptr) = pixel_request.get_color();
  ...
  glReadPixels(static_cast<GLint>(x), static_cast<GLint>(y), 1, 1, GL_RGB,
               GL_FLOAT, pixels);
...
void GLPainter::set_clip_rect(const Rect& clip_rect) {
  assert_gl();
...
void GLPainter::clear_clip_rect() {
  assert_gl();
