...
TextureManager::TextureManager() : m_image_textures(), m_surfaces() {}
...
TexturePtr TextureManager::get(const ReaderMapping& mapping,
                               const boost::optional<Rect>& region) {
  std::string filename;
...
void TextureManager::reap_cache_entry(const Texture::Key& key) {
  auto i = m_image_textures.find(key);
...
TexturePtr TextureManager::create_image_texture_raw(const std::string& filename,
  ...
  if (src_surface.format->Rmask == 0 && src_surface.format->Gmask == 0 &&
      src_surface.format->Bmask == 0 && src_surface.format->Amask == 0) {
    log_debug << "Wrong surface format for image " << filename
              << ". Compensating." << std::endl;
    convert.reset(SDL_ConvertSurfaceFormat(
        const_cast<SDL_Surface*>(&src_surface), SDL_PIXELFORMAT_RGBA8888, 0));
  }

  const SDL_Surface& surface = convert ? *convert : src_surface;

  SDLSurfacePtr subimage(SDL_CreateRGBSurfaceFrom(
      static_cast<uint8_t*>(surface.pixels) + rect.top * surface.pitch +
          rect.left * surface.format->BytesPerPixel,
      rect.get_width(), rect.get_height(), surface.format->BitsPerPixel,
      surface.pitch, surface.format->Rmask, surface.format->Gmask,
...
void TextureManager::debug_print(std::ostream& out) const {
  ...
  for (const auto& it : m_surfaces) {
    ...
    out << "  surface filename:" << filename << " " << surface->w << "x"
        << surface->h << std::endl;
  }
