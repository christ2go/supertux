...
inline Size apply_pixel_aspect_ratio_pre(const Size& window_size,
                                         float pixel_aspect_ratio) {
  return Size(static_cast<int>(static_cast<float>(window_size.width) *
                               pixel_aspect_ratio),
              window_size.height);
}

inline void apply_pixel_aspect_ratio_post(const Size& real_window_size,
                                          const Size& window_size, float scale,
                                          Rect& out_viewport,
                                          Vector& out_scale) {
  Vector transform(static_cast<float>(real_window_size.width) /
                       static_cast<float>(window_size.width),
                   static_cast<float>(real_window_size.height) /
                       static_cast<float>(window_size.height));

  out_viewport.left =
      static_cast<int>(static_cast<float>(out_viewport.left) * transform.x);
  out_viewport.top =
      static_cast<int>(static_cast<float>(out_viewport.top) * transform.y);
  out_viewport.right =
      static_cast<int>(static_cast<float>(out_viewport.right) * transform.x);
  out_viewport.bottom =
      static_cast<int>(static_cast<float>(out_viewport.bottom) * transform.y);

  out_scale.x = scale * transform.x;
  out_scale.y = scale * transform.y;
}

inline float calculate_scale(const Size& min_size, const Size& max_size,
  ...
  if (scale == 0.0f)  // magic value
  {
    ...
    if (window_size.width > max_size.width ||
      ...
      scale = std::max(static_cast<float>(window_size.width) /
                           static_cast<float>(max_size.width),
                       static_cast<float>(window_size.height) /
                           static_cast<float>(max_size.height));
    }

    // If the resulting area would violate min_size, scale it down
    if (static_cast<float>(window_size.width) / scale <
            static_cast<float>(min_size.width) ||
        static_cast<float>(window_size.height) / scale <
            static_cast<float>(min_size.height)) {
      scale = std::min(static_cast<float>(window_size.width) /
                           static_cast<float>(min_size.width),
                       static_cast<float>(window_size.height) /
                           static_cast<float>(min_size.height));
    }
...
inline Rect calculate_viewport(const Size& max_size, const Size& window_size,
                               float scale) {
  int viewport_width =
      std::min(window_size.width,
               static_cast<int>(scale * static_cast<float>(max_size.width)));
  int viewport_height =
      std::min(window_size.height,
...
Viewport Viewport::from_size(const Size& target_size,
                             const Size& desktop_size) {
  float pixel_aspect_ratio = 1.0f;
  if (g_config->aspect_size != Size(0, 0)) {
    pixel_aspect_ratio =
        calculate_pixel_aspect_ratio(desktop_size, g_config->aspect_size);
  } else if (g_config->use_fullscreen) {
    pixel_aspect_ratio =
        calculate_pixel_aspect_ratio(desktop_size, target_size);
  }

  // calculate the viewport
  Rect viewport;
  Vector scale;
  calculate_viewport(s_min_size, s_max_size, target_size, pixel_aspect_ratio,
                     g_config->magnification, scale, viewport);

  return Viewport(viewport, scale);
}

Viewport::Viewport() : m_rect(), m_scale() {}
...
bool Viewport::needs_clear_screen() const {
