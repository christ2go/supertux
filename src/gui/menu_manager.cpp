...
class MenuTransition final {
 ...
 public:
  ...
  void draw(DrawingContext& context) {
    ...
    if (m_is_active) {
      rect = Rectf(
          (m_to_rect.get_left() * p) + (m_from_rect.get_left() * (1.0f - p)),
          (m_to_rect.get_top() * p) + (m_from_rect.get_top() * (1.0f - p)),
          (m_to_rect.get_right() * p) + (m_from_rect.get_right() * (1.0f - p)),
          (m_to_rect.get_bottom() * p) +
              (m_from_rect.get_bottom() * (1.0f - p)));
    }

    // draw menu background rectangles
    context.color().draw_filled_rect(
        Rectf(rect.get_left() - 4, rect.get_top() - 10 - 4,
              rect.get_right() + 4, rect.get_bottom() + 10 + 4),
        Color(0.2f, 0.3f, 0.4f, 0.8f), 20.0f, LAYER_GUI - 10);

    context.color().draw_filled_rect(
        Rectf(rect.get_left(), rect.get_top() - 10, rect.get_right(),
              rect.get_bottom() + 10),
...
void MenuManager::event(const SDL_Event& ev) {
...
void MenuManager::draw(DrawingContext& context) {
...
void MenuManager::set_dialog(std::unique_ptr<Dialog> dialog) {
...
void MenuManager::push_menu(int id) {
  push_menu(
...
void MenuManager::push_menu(std::unique_ptr<Menu> menu) {
  assert(menu);
...
void MenuManager::on_window_resize() {
  for (const auto& menu : m_menu_stack) {
