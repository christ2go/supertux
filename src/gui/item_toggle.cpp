...
ItemToggle::ItemToggle(const std::string& text_, std::function<bool()> get_func,
                       std::function<void(bool)> set_func, int id_)
    : MenuItem(text_, id_),
      m_get_func(std::move(get_func)),
      m_set_func(std::move(set_func)) {}

void ItemToggle::draw(DrawingContext& context, const Vector& pos,
  ...
  if (m_get_func()) {
    context.color().draw_surface(
        ...
        Vector(pos.x + static_cast<float>(menu_width) - 16.0f -
                   static_cast<float>(Resources::checkbox->get_width()),
               pos.y - 8.0f),
        LAYER_GUI);
  } else {
    context.color().draw_surface(
...
int ItemToggle::get_width() const {
  return static_cast<int>(Resources::normal_font->get_text_width(get_text())) +
         16 + Resources::checkbox->get_width();
}

void ItemToggle::process_action(const MenuAction& action) {
