...
void ItemTextField::draw(DrawingContext& context, const Vector& pos,
...
int ItemTextField::get_width() const {
  return static_cast<int>(Resources::normal_font->get_text_width(get_text()) +
                          Resources::normal_font->get_text_width(*input) +
                          16.0f + static_cast<float>(flickw));
}

void ItemTextField::event(const SDL_Event& ev) {
  if (ev.type == SDL_TEXTINPUT) {
    *input += ev.text.text;
  }
}

void ItemTextField::process_action(const MenuAction& action) {
