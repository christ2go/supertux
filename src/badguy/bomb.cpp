...
Bomb::Bomb(const Vector& pos, Direction dir_,
           const std::string&
               custom_sprite /*= "images/creatures/mr_bomb/mr_bomb.sprite"*/)
...
void Bomb::collision_solid(const CollisionHit& hit) {
  if (is_grabbed()) {
    return;
  }
  if (hit.top || hit.bottom) m_physic.set_velocity_y(0);
  if (hit.left || hit.right)
    m_physic.set_velocity_x(-m_physic.get_velocity_x());
  if (hit.crush) m_physic.set_velocity(0, 0);

  update_on_ground_flag(hit);
}

HitResponse Bomb::collision_player(Player&, const CollisionHit&) {
  return ABORT_MOVE;
}

HitResponse Bomb::collision_badguy(BadGuy&, const CollisionHit&) {
  return ABORT_MOVE;
}

void Bomb::active_update(float dt_sec) {
...
void Bomb::kill_fall() { explode(); }

void Bomb::ignite() { explode(); }

void Bomb::grab(MovingObject& object, const Vector& pos, Direction dir_) {
...
void Bomb::ungrab(MovingObject& object, Direction dir_) {
  m_dir = dir_;
    ...
    if (player && player->get_physic().get_velocity_x() *
      ...
      toss_velocity_x +=
          static_cast<int>(player->get_physic().get_velocity_x() -
                           (190.0f * (dir_ == Direction::LEFT ? -1.0f : 1.0f)));
  }
