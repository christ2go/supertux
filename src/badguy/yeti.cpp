...
void Yeti::draw(DrawingContext& context) {
  // we blink when we are safe
  if (safe_timer.started() && size_t(g_game_time * 40) % 2) return;
...
void Yeti::draw_hit_points(DrawingContext& context) {
  if (hud_head) {
    ...
    for (int i = 0; i < hit_points; ++i) {
      context.color().draw_surface(
          hud_head,
          Vector(BORDER_X + (static_cast<float>(i * hud_head->get_width())),
                 BORDER_Y + 1),
          LAYER_FOREGROUND1);
    }
...
void Yeti::active_update(float dt_sec) {
  switch (state) {
    case JUMP_DOWN:
      m_physic.set_velocity_x((m_dir == Direction::RIGHT) ? +JUMP_DOWN_VX
                                                          : -JUMP_DOWN_VX);
      break;
    case RUN:
      m_physic.set_velocity_x((m_dir == Direction::RIGHT) ? +RUN_VX : -RUN_VX);
      if (((m_dir == Direction::RIGHT) && (get_pos().x >= right_jump_x)) ||
          ((m_dir == Direction::LEFT) && (get_pos().x <= left_jump_x)))
        jump_up();
      break;
    case JUMP_UP:
      m_physic.set_velocity_x((m_dir == Direction::RIGHT) ? +JUMP_UP_VX
                                                          : -JUMP_UP_VX);
      if (((m_dir == Direction::RIGHT) && (get_pos().x >= right_stand_x)) ||
          ((m_dir == Direction::LEFT) && (get_pos().x <= left_stand_x)))
        be_angry();
      break;
    case BE_ANGRY:
      if (state_timer.check() && on_ground()) {
        m_physic.set_velocity_y(STOMP_VY);
        m_sprite->set_action((m_dir == Direction::RIGHT) ? "stomp-right"
                                                         : "stomp-left");
    ...
    case SQUISHED: {
      ...
      m_dir = newdir;
      m_sprite->set_action((m_dir == Direction::RIGHT) ? "jump-right"
                                                       : "jump-left");
    }
      if (state_timer.check()) {
        ...
        m_physic.set_velocity_y(JUMP_UP_VY /
                                2);  // Move up a bit before falling
...
void Yeti::run() {
  m_sprite->set_action((m_dir == Direction::RIGHT) ? "walking-right"
                                                   : "walking-left");
  m_physic.set_velocity_x((m_dir == Direction::RIGHT) ? (+RUN_VX) : (-RUN_VX));
...
void Yeti::be_angry() {
  ...
  m_sprite->set_action((m_dir == Direction::RIGHT) ? "stand-right"
                                                   : "stand-left");
...
bool Yeti::collision_squished(GameObject& object) {
  ...
  for (auto& stalactite : Sector::get().get_objects_by_type<YetiStalactite>()) {
    if (stalactite.is_hanging()) {
      if (hit_points >= 3) {
        ...
        if ((((static_cast<int>(stalactite.get_pos().x) + 16) / 64) % 3) ==
            (stomp_count % 3)) {
          stalactite.start_shaking();
        }
      }
...
void Yeti::collision_solid(const CollisionHit& hit) {
  ...
  if (hit.top || hit.bottom) {
    ...
    // hit wall
    if (state != SQUISHED && state != FALLING) jump_up();
  }
}

bool Yeti::is_flammable() const { return false; }

ObjectSettings Yeti::get_settings() {
  ObjectSettings result = BadGuy::get_settings();

  result.add_text("hud-icon", &hud_icon, "hud-icon",
                  std::string("images/creatures/yeti/hudlife.png"),
                  OPTION_HIDDEN);
...
void Yeti::add_snow_explosions() {
  for (int i = 0; i < SNOW_EXPLOSIONS_COUNT; i++) {
    ...
    velocity.x = SNOW_EXPLOSIONS_VX * graphicsRandom.randf(0.5f, 2.0f) *
                 (graphicsRandom.rand(2) ? 1.0f : -1.0f);
    velocity.y = SNOW_EXPLOSIONS_VY * graphicsRandom.randf(0.5f, 2.0f);
    pos.x += static_cast<float>(m_sprite->get_width()) / 2.0f;
    pos.x += static_cast<float>(m_sprite->get_width()) *
             graphicsRandom.randf(0.3f, 0.5f) *
             ((velocity.x > 0) ? 1.0f : -1.0f);
    pos.y += static_cast<float>(m_sprite->get_height()) *
             graphicsRandom.randf(-0.3f, 0.3f);
...
Yeti::SnowExplosionParticle::SnowExplosionParticle(const Vector& pos,
    ...
    : BadGuy(pos, (velocity.x > 0) ? Direction::RIGHT : Direction::LEFT,
