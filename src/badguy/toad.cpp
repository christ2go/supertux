...
void Toad::initialize() {
  // initial state is JUMPING, because we might start airborne
  state = JUMPING;
  m_sprite->set_action(m_dir == Direction::LEFT ? "jumping-left"
                                                : "jumping-right");
}

void Toad::set_state(ToadState newState) {
  if (newState == IDLE) {
    m_physic.set_velocity_x(0);
    m_physic.set_velocity_y(0);
    if (!m_frozen)
      m_sprite->set_action(m_dir == Direction::LEFT ? "idle-left"
                                                    : "idle-right");
    ...
    m_sprite->set_action(m_dir == Direction::LEFT ? "jumping-left"
                                                  : "jumping-right");
    m_physic.set_velocity_x(m_dir == Direction::LEFT ? -HORIZONTAL_SPEED
                                                     : HORIZONTAL_SPEED);
    ...
    // face player
    if (player && (player->get_bbox().get_right() < m_col.m_bbox.get_left()) &&
        (m_dir == Direction::RIGHT))
      m_dir = Direction::LEFT;
    if (player && (player->get_bbox().get_left() > m_col.m_bbox.get_right()) &&
        (m_dir == Direction::LEFT))
      m_dir = Direction::RIGHT;
...
bool Toad::collision_squished(GameObject& object) {
...
void Toad::collision_solid(const CollisionHit& hit) {
  ...
  if (((m_physic.get_velocity_x() < 0) && hit.left) ||
      ((m_physic.get_velocity_x() > 0) && hit.right)) {
    /*
...
HitResponse Toad::collision_badguy(BadGuy&, const CollisionHit& hit) {
...
void Toad::active_update(float dt_sec) {
...
bool Toad::is_freezable() const { return true; }
