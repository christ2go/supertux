...
bool Haywire::collision_squished(GameObject& object) {
...
void Haywire::active_update(float dt_sec) {
  if (is_exploding) {
...
bool Haywire::is_freezable() const { return true; }

void Haywire::ignite() { kill_fall(); }
...
void Haywire::start_exploding() {
  set_action((m_dir == Direction::LEFT) ? "ticking-left" : "ticking-right",
             /* loops = */ -1);
