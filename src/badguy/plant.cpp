...
void Plant::collision_solid(const CollisionHit& hit) {
...
HitResponse Plant::collision_badguy(BadGuy&, const CollisionHit& hit) {
...
void Plant::active_update(float dt_sec) {
  ...
  if (state == PLANT_SLEEPING) {
    ...
    if (player) {
      ...
      if (inReach_left && inReach_right && inReach_top && inReach_bottom) {
        // wake up
        m_sprite->set_action(m_dir == Direction::LEFT ? "waking-left"
                                                      : "waking-right");
  ...
  if (state == PLANT_WAKING) {
    if (timer.check()) {
      // start walking
      m_sprite->set_action(m_dir == Direction::LEFT ? "left" : "right");
      m_physic.set_velocity_x(m_dir == Direction::LEFT ? -PLANT_SPEED
                                                       : PLANT_SPEED);
      state = PLANT_WALKING;
    }
  }
...
void Plant::ignite() {
  BadGuy::ignite();
  if (state == PLANT_SLEEPING &&
      m_sprite->has_action("sleeping-burning-left")) {
    m_sprite->set_action(m_dir == Direction::LEFT ? "sleeping-burning-left"
                                                  : "sleeping-burning-right",
                         1);
  }
}
/* EOF */
