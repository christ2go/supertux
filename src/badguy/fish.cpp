...
void Fish::collision_solid(const CollisionHit& chit) { hit(chit); }

HitResponse Fish::collision_badguy(BadGuy&, const CollisionHit& chit) {
  return hit(chit);
}

void Fish::draw(DrawingContext& context) {
  if (waiting.started()) return;

  BadGuy::draw(context);
}

HitResponse Fish::hit(const CollisionHit& hit_) {
  if (hit_.top) {
...
void Fish::collision_tile(uint32_t tile_attributes) {
  if ((tile_attributes & Tile::WATER) && (m_physic.get_velocity_y() >= 0)) {
    ...
    if (get_pos().y >= stop_y) {
      if (!m_frozen) start_waiting();
      m_col.m_movement = Vector(0, 0);
    }
  }
  if ((!(tile_attributes & Tile::WATER) || m_frozen) &&
      (tile_attributes & Tile::HURTS)) {
    kill_fall();
  }
}

void Fish::active_update(float dt_sec) {
...
void Fish::unfreeze() {  // does this happen at all? (or do fishes die when they
                         // fall frozen?)
  BadGuy::unfreeze();
  start_waiting();
}
...
bool Fish::is_freezable() const { return true; }
