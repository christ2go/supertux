...
void Dart::initialize() {
  m_physic.set_velocity_x(m_dir == Direction::LEFT ? -::DART_SPEED
                                                   : ::DART_SPEED);
  m_sprite->set_action(m_dir == Direction::LEFT ? "flying-left"
                                                : "flying-right");
}
...
void Dart::deactivate() {
  sound_source.reset();
  remove_me();
}

void Dart::active_update(float dt_sec) {
  BadGuy::active_update(dt_sec);
  sound_source->set_position(get_pos());
}

void Dart::collision_solid(const CollisionHit&) {
  SoundManager::current()->play("sounds/darthit.wav", get_pos());
  remove_me();
}

HitResponse Dart::collision_badguy(BadGuy& badguy, const CollisionHit&) {
...
HitResponse Dart::collision_player(Player& player, const CollisionHit& hit) {
  SoundManager::current()->play("sounds/stomp.wav", get_pos());
  remove_me();
  return BadGuy::collision_player(player, hit);
}

bool Dart::is_flammable() const { return false; }
