...
Dispenser::DispenserType Dispenser::DispenserType_from_string(
...
std::string Dispenser::DispenserType_to_string(DispenserType type) {
  switch (type) {
...
Dispenser::Dispenser(const ReaderMapping& reader)
  ...
  switch (m_type) {
    ...
    case DispenserType::ROCKETLAUNCHER:
      m_sprite->set_action(m_dir == Direction::LEFT ? "working-left"
                                                    : "working-right");
...
void Dispenser::draw(DrawingContext& context) {
...
void Dispenser::deactivate() { m_dispense_timer.stop(); }

// TODO: Add launching velocity to certain badguys
bool Dispenser::collision_squished(GameObject& object) {
  ...
  m_sprite->set_action(m_dir == Direction::LEFT ? "broken-left"
                                                : "broken-right");
...
HitResponse Dispenser::collision(GameObject& other, const CollisionHit& hit) {
...
void Dispenser::active_update(float) {
  if (m_dispense_timer.check()) {
    ...
    if (m_autotarget) {
      ...
      auto player = get_nearest_player();
      if (player && !m_swivel) {
        Direction targetdir = (player->get_pos().x > get_pos().x)
                                  ? Direction::RIGHT
                                  : Direction::LEFT;
        if (m_dir != targetdir) {  // no target: swivel cannon
          ...
          m_dir = targetdir;
          m_sprite->set_action(
              m_dir == Direction::LEFT ? "swivel-left" : "swivel-right", 1);
...
void Dispenser::launch_badguy() {
  ...
  if (!is_offscreen() && !Editor::is_active()) {
    ...
    if (!m_autotarget && m_start_dir == Direction::AUTO) {
      ...
      if (player) {
        launchdir = (player->get_pos().x > get_pos().x) ? Direction::RIGHT
                                                        : Direction::LEFT;
      }
    }
    ...
    if (m_badguys.size() > 1) {
      if (m_random) {
        m_next_badguy = static_cast<unsigned int>(
            gameRandom.rand(static_cast<int>(m_badguys.size())));
      } else {
        m_next_badguy++;

        if (m_next_badguy >= m_badguys.size()) m_next_badguy = 0;
      }
    }
    ...
    try {
      ...
      auto game_object =
          GameObjectFactory::instance().create(badguy, get_pos(), launchdir);
...
void Dispenser::freeze() {
  ...
  if (m_type == DispenserType::ROCKETLAUNCHER &&
      m_sprite->has_action("iced-left"))
...
bool Dispenser::is_freezable() const { return true; }

bool Dispenser::is_flammable() const { return false; }

void Dispenser::set_correct_action() {
  switch (m_type) {
    case DispenserType::DROPPER:
      m_sprite->set_action("dropper");
      break;
...
ObjectSettings Dispenser::get_settings() {
  ...
  result.add_enum(
      _("Type"), reinterpret_cast<int*>(&m_type),
  ...
  result.reorder({"cycle", "random", "type", "badguy", "direction",
                  "limit-dispensed-badguys", "max-concurrent-badguys", "x",
                  "y"});

  return result;
}

void Dispenser::after_editor_set() { set_correct_action(); }
