...
void BadGuy::draw(DrawingContext& context) {
...
void BadGuy::update(float dt_sec) {
  if (!Sector::get().inside(m_col.m_bbox)) {
    ...
    if (!this_portable || !this_portable->is_grabbed()) {
      ...
      remove_me();
      // This was removed due to fixing a bug. If is it needed somewhere, then
      // I'm sorry. --Hume2
      /**if(countMe) {
  ...
  switch (m_state) {
    ...
    case STATE_MELTING: {
      ...
      if (m_sprite->animation_done() || on_ground()) {
        Sector::get().add<WaterDrop>(m_col.m_bbox.p1(), get_water_sprite(),
                                     m_physic.get_velocity());
        remove_me();
        break;
      }
    } break;
    ...
    case STATE_INSIDE_MELTING: {
      ...
      if (on_ground() && m_sprite->animation_done()) {
        m_sprite->set_action(
            m_dir == Direction::LEFT ? "gear-left" : "gear-right", 1);
      ...
      float py = graphicsRandom.randf(m_col.m_bbox.get_top(),
      ...
      Sector::get().add<SpriteParticle>(
          ...
          Vector(0, 100 * Sector::get().get_gravity()), LAYER_OBJECTS - 1);
    } break;
...
Direction BadGuy::str2dir(const std::string& dir_str) const {
  if (dir_str == "auto") return Direction::AUTO;
  if (dir_str == "left") return Direction::LEFT;
  if (dir_str == "right") return Direction::RIGHT;
...
void BadGuy::activate() {}

void BadGuy::deactivate() {}

void BadGuy::active_update(float dt_sec) {
  m_col.m_movement = m_physic.get_movement(dt_sec);
  if (m_frozen) m_sprite->stop_animation();
}

void BadGuy::inactive_update(float) {}
...
HitResponse BadGuy::collision(GameObject& other, const CollisionHit& hit) {
  ...
  if (badguy && badguy->is_active() &&
      badguy->m_col.get_group() == COLGROUP_MOVING) {
...
HitResponse BadGuy::collision_player(Player& player, const CollisionHit&) {
...
HitResponse BadGuy::collision_badguy(BadGuy&, const CollisionHit&) {
  return FORCE_MOVE;
}

bool BadGuy::collision_squished(GameObject& object) {
...
HitResponse BadGuy::collision_bullet(Bullet& bullet, const CollisionHit& hit) {
  if (is_frozen()) {
...
void BadGuy::kill_squished(GameObject& object) {
  if (!is_active()) return;
    ...
    for (pr_pos.x = 0; pr_pos.x < m_col.m_bbox.get_width(); pr_pos.x += 16) {
      for (pr_pos.y = 0; pr_pos.y < m_col.m_bbox.get_height(); pr_pos.y += 16) {
        ...
        Sector::get().add<SpriteParticle>(
            ...
            m_col.m_bbox.p1() + pr_pos, ANCHOR_MIDDLE, speed,
...
void BadGuy::run_dead_script() {
  if (m_countMe) Sector::get().get_level().m_stats.m_badguys++;
...
void BadGuy::set_state(State state_) {
  if (m_state == state_) return;
...
bool BadGuy::is_offscreen() const {
  Vector cam_dist;
  Vector player_dist;
  Camera& cam = Sector::get().get_camera();
  cam_dist = cam.get_center() - m_col.m_bbox.get_middle();
  if (Editor::is_active()) {
    if ((fabsf(cam_dist.x) <= X_OFFSCREEN_DISTANCE) &&
        (fabsf(cam_dist.y) <= Y_OFFSCREEN_DISTANCE)) {
      return false;
    }
  }
  ...
  if (((fabsf(player_dist.x) <= X_OFFSCREEN_DISTANCE) &&
       (fabsf(player_dist.y) <= Y_OFFSCREEN_DISTANCE)) ||
      ((fabsf(cam_dist.x) <= X_OFFSCREEN_DISTANCE) &&
       (fabsf(cam_dist.y) <= Y_OFFSCREEN_DISTANCE))) {
    return false;
  }
  return true;
  ...
  if (!is_offscreen()) {
    ...
    if (!m_is_initialized) {
      ...
      if (m_start_dir == Direction::AUTO) {
        ...
        if (player_ &&
            (player_->get_bbox().get_left() > m_col.m_bbox.get_right())) {
...
bool BadGuy::might_fall(int height) const {
...
Player* BadGuy::get_nearest_player() const {
  return Sector::get().get_nearest_player(m_col.m_bbox);
}

void BadGuy::update_on_ground_flag(const CollisionHit& hit) {
  if (hit.bottom) {
...
bool BadGuy::on_ground() const { return m_on_ground_flag; }

bool BadGuy::is_active() const { return m_is_active_flag; }

Vector BadGuy::get_floor_normal() const { return m_floor_normal; }
...
void BadGuy::freeze() {
  ...
  if (m_sprite->has_action("iced-left"))
    m_sprite->set_action(m_dir == Direction::LEFT ? "iced-left" : "iced-right",
                         1);
  // when the sprite doesn't have separate actions for left and right, it tries
  // to use an universal one.
  else {
    if (m_sprite->has_action("iced")) m_sprite->set_action("iced", 1);
...
bool BadGuy::is_freezable() const { return false; }

bool BadGuy::is_frozen() const { return m_frozen; }

bool BadGuy::is_in_water() const { return m_in_water; }
...
void BadGuy::ignite() {
  ...
  if (m_sprite->has_action("melting-left")) {
    ...
    if (m_sprite->has_action("ground-melting-left") && on_ground()) {
      m_sprite->set_action(m_dir == Direction::LEFT ? "ground-melting-left"
                                                    : "ground-melting-right",
                           1);
      SoundManager::current()->play("sounds/splash.ogg", get_pos());
      set_state(STATE_GROUND_MELTING);
    } else {
      m_sprite->set_action(
          m_dir == Direction::LEFT ? "melting-left" : "melting-right", 1);
    ...
    m_sprite->set_action(
        m_dir == Direction::LEFT ? "burning-left" : "burning-right", 1);
    set_state(STATE_BURNING);
    run_dead_script();
  } else if (m_sprite->has_action("inside-melting-left")) {
    // melt it inside!
    SoundManager::current()->play("sounds/splash.ogg", get_pos());
    m_sprite->set_action(m_dir == Direction::LEFT ? "inside-melting-left"
                                                  : "inside-melting-right",
                         1);
...
void BadGuy::extinguish() {}

bool BadGuy::is_flammable() const { return true; }

bool BadGuy::is_ignited() const { return m_ignited; }

void BadGuy::set_colgroup_active(CollisionGroup group_) {
  m_colgroup_active = group_;
  if (m_state == STATE_ACTIVE) set_group(group_);
}

ObjectSettings BadGuy::get_settings() {
  ObjectSettings result = MovingSprite::get_settings();

  result.add_direction(_("Direction"), &m_start_dir, Direction::AUTO,
                       "direction");
...
void BadGuy::after_editor_set() {
  if (m_dir == Direction::AUTO) {
    if (m_sprite->has_action("editor-left")) {
      ...
      log_warning << "couldn't find editor sprite for badguy direction='auto': "
                  << get_class() << std::endl;
    }
  } else {
    ...
    if (m_sprite->has_action("editor-" + action_str)) {
      ...
      log_warning << "couldn't find editor sprite for badguy direction='"
                  << action_str << "': " << get_class() << std::endl;
    }
  }
}
