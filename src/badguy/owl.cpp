...
void Owl::initialize() {
  m_physic.set_velocity_x(m_dir == Direction::LEFT ? -FLYING_SPEED
                                                   : FLYING_SPEED);
  ...
  auto game_object =
      GameObjectFactory::instance().create(carried_obj_name, get_pos(), m_dir);
...
bool Owl::is_above_player() const {
  ...
  float x_offset =
      (m_dir == Direction::LEFT) ? ACTIVATION_DISTANCE : -ACTIVATION_DISTANCE;

  const Rectf& player_bbox = player->get_bbox();

  return ((player_bbox.get_top() >=
           m_col.m_bbox.get_bottom()) /* player is below us */
          && ((player_bbox.get_right() + x_offset) > m_col.m_bbox.get_left()) &&
          ((player_bbox.get_left() + x_offset) < m_col.m_bbox.get_right()));
}

void Owl::active_update(float dt_sec) {
  ...
  if (carried_object != nullptr) {
    if (!is_above_player()) {
      ...
      obj_pos.x -=
          16.f; /* FIXME: Actually do use the half width of the carried object
                   here. */
      obj_pos.y +=
          3.f; /* Move a little away from the hitbox (the body). Looks nicer. */
...
bool Owl::collision_squished(GameObject&) {
  auto player = Sector::get().get_nearest_player(m_col.m_bbox);
...
bool Owl::is_freezable() const { return true; }

void Owl::collision_solid(const CollisionHit& hit) {
  if (m_frozen) {
...
ObjectSettings Owl::get_settings() {
  ObjectSettings result = BadGuy::get_settings();

  result.add_text(_("Carry"), &carried_obj_name,
                  "carry");  //, std::string("skydive"));
