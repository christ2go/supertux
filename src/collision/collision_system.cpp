...
void CollisionSystem::draw(DrawingContext& context) {
...
void CollisionSystem::collision_tilemap(collision::Constraints* constraints,
  ...
  for (const auto& solids : m_sector.get_solid_tilemaps()) {
    ...
    const Rect test_tiles =
        solids->get_tiles_overlapping(Rectf(x1, y1, x2, y2));
    ...
    for (int x = test_tiles.left; x < test_tiles.right; ++x) {
      for (int y = test_tiles.top; y < test_tiles.bottom; ++y) {
        ...
        if (tile.is_slope()) {  // slope tile
          ...
          check_collisions(constraints, movement, dest, tile_bbox, nullptr,
                           nullptr, solids->get_movement(/* actual = */ false));
        }
      }
    }
  }
...
uint32_t CollisionSystem::collision_tile_attributes(const Rectf& dest,
...
void CollisionSystem::collision_object(CollisionObject* object1,
                                       CollisionObject* object2) const {
...
void CollisionSystem::collision_static(collision::Constraints* constraints,
  ...
  for (auto& static_object : m_objects) {
    if (static_object->get_group() != COLGROUP_STATIC &&
      ...
      continue;
    if (!static_object->is_valid()) continue;
...
void CollisionSystem::collision_static_constrains(CollisionObject& object) {
  using namespace collision;

  const float infinity = (std::numeric_limits<float>::has_infinity
...
void CollisionSystem::update() {
  ...
  for (const auto& object : m_objects) {
    ...
    if (((mov.x > MAX_SPEED * static_cast<float>(M_SQRT1_2)) ||
         (mov.y > MAX_SPEED * static_cast<float>(M_SQRT1_2))) &&
        (mov.norm() > MAX_SPEED)) {
      object->m_movement = mov.unit() * MAX_SPEED;
      // log_debug << "Temporarily reduced object's speed of " << mov.norm() <<
      // " to " << object->movement.norm() << "." << std::endl;
    }
  ...
  for (const auto& object : m_objects) {
    ...
    uint32_t tile_attributes =
        collision_tile_attributes(object->m_dest, object->get_movement());
  ...
  for (const auto& object : m_objects) {
    ...
    for (auto& object_2 : m_objects) {
      if (object_2->get_group() != COLGROUP_TOUCHABLE || !object_2->is_valid())
        continue;

      if (intersects(object->m_dest, object_2->m_dest)) {
        Vector normal;
        CollisionHit hit;
        get_hit_normal(object->m_dest, object_2->m_dest, hit, normal);
        if (!object->collides(*object_2, hit)) continue;
        if (!object_2->collides(*object, hit)) continue;
...
bool CollisionSystem::is_free_of_tiles(const Rectf& rect,
  ...
  for (const auto& solids : m_sector.get_solid_tilemaps()) {
    ...
    for (int x = test_tiles.left; x < test_tiles.right; ++x) {
      for (int y = test_tiles.top; y < test_tiles.bottom; ++y) {
        ...
        if (!(tile.get_attributes() & Tile::SOLID)) continue;
...
bool CollisionSystem::is_free_of_statics(const Rectf& rect,
...
bool CollisionSystem::is_free_of_movingstatics(
    const Rectf& rect, const CollisionObject* ignore_object) const {
  using namespace collision;
...
bool CollisionSystem::free_line_of_sight(
    const Vector& line_start, const Vector& line_end,
