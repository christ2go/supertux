...
void ConsoleBuffer::set_console(Console* console) {
  assert((console && !m_console) || (m_console && !console));

  m_console = console;
}

void ConsoleBuffer::addLines(const std::string& s) {
...
void ConsoleBuffer::addLine(const std::string& s_) {
  std::string s = s_;
...
void ConsoleBuffer::flush(ConsoleStreamBuffer& buffer) {
...
Console::~Console() {
  if (m_vm != nullptr && SquirrelVirtualMachine::current() != nullptr) {
    sq_release(SquirrelVirtualMachine::current()->get_vm().get_vm(),
               &m_vm_object);
  }
  m_buffer.set_console(nullptr);
}

void Console::on_buffer_change(int line_count) {
...
void Console::execute_script(const std::string& command) {
  ready_vm();
...
void Console::scroll(int numLines) {
  m_offset += numLines;
  if (m_offset > 0) m_offset = 0;
}

void Console::show_history(int offset_) {
...
void Console::move_cursor(int offset_) {
 ...
 */
void sq_insert_command(std::list<std::string>& cmds, HSQUIRRELVM vm,
                       const std::string& table_prefix,
                       const std::string& search_prefix) {
...
void Console::autocomplete() {
  ...
  // m_inputBufferPosition);
  int autocompleteFrom = static_cast<int>(m_inputBuffer.find_last_not_of(
  ...
  if (cmds.size() == 1) {
    ...
    m_inputBufferPosition +=
        static_cast<int>(replaceWith.length() - prefix.length());
  }
...
void Console::parse(const std::string& s) {
...
bool Console::consoleCommand(const std::string& /*command*/,
                             const std::vector<std::string>& /*arguments*/) {
  return false;
}

bool Console::hasFocus() const { return m_focused; }
...
void Console::open() {
  if (m_stayOpen < 2) m_stayOpen += 1.5f;
}
...
void Console::update(float dt_sec) {
  if (m_stayOpen > 0) {
    m_stayOpen -= dt_sec;
  ...
  if (m_backgroundOffset > static_cast<int>(m_background->get_width()))
    m_backgroundOffset -= static_cast<int>(m_background->get_width());
}

void Console::draw(DrawingContext& context) const {
  ...
  context.color().draw_surface(
      m_background2,
      Vector(static_cast<float>(context_center_x - background_center_x -
  ...
  for (int x = (context_center_x - background_center_x -
                (static_cast<int>(
                     ceilf(static_cast<float>(context.get_width()) /
                           static_cast<float>(m_background->get_width())) -
                     1) *
                 m_background->get_width()));
       x < context.get_width(); x += m_background->get_width()) {
    context.color().draw_surface(
        m_background,
  ...
  for (std::list<std::string>::iterator i = m_buffer.m_lines.begin();
    ...
    float py = static_cast<float>(
        m_height - 4.0f - static_cast<float>(lineNo) * m_font->get_height());
