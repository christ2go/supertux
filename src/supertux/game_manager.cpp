...
GameManager::GameManager()
    : m_savegame(), m_next_worldmap(), m_next_spawnpoint() {}

void GameManager::start_level(const World& world,
  ...
  auto screen = std::make_unique<LevelsetScreen>(world.get_basedir(),
                                                 level_filename, *m_savegame);
  ScreenManager::current()->push_screen(std::move(screen));
}

void GameManager::start_worldmap(const World& world,
  ...
  try {
    ...
    auto worldmap =
        std::make_unique<worldmap::WorldMap>(filename, *m_savegame, spawnpoint);
    auto worldmap_screen =
        std::make_unique<worldmap::WorldMapScreen>(std::move(worldmap));
...
bool GameManager::load_next_worldmap() {
...
void GameManager::set_next_worldmap(const std::string& worldmap,
                                    const std::string& spawnpoint) {
