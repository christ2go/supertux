...
ContribMenu::ContribMenu() : m_contrib_worlds() {
  ...
  for (const char* const* addondir = addons.get(); *addondir != nullptr;
       ++addondir) {
    ...
    if (physfsutil::is_directory(addonpath)) {
      ...
      if (physfsutil::is_directory(addonlevelpath)) {
        ...
        for (const char* const* filename = addonfiles.get();
    ...
    try {
      ...
      if (!world->hide_from_contribs()) {
        ...
        if (world->is_levelset()) {
          ...
          const auto& state =
              savegame->get_levelset_state(world->get_basedir());
...
void ContribMenu::menu_action(MenuItem& item) {
  ...
  if (index != -1) {
    ...
    std::unique_ptr<World> world =
        World::from_directory(m_contrib_worlds[index]->get_basedir());
    if (!world->is_levelset()) {
