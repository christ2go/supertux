...
DebugMenu::DebugMenu() : next_game_speed(0) {
  add_label(_("Debug"));
  add_hl();
  ...
  {  // game speed menu entry
    ...
    for (int percent : {0, 5, 10, 25, 50, 75, 100, 150, 200, 300, 400, 500,
                        1000, 1500, 3000}) {
    ...
    auto it =
        std::find(game_speed_multiplier.begin(), game_speed_multiplier.end(),
                  g_debug.get_game_speed_multiplier());
    ...
    auto& item = add_string_select(MNID_GAME_SPEED, _("Game Speed"),
                                   &next_game_speed, game_speeds);
  ...
  add_entry(_("Dump Texture Cache"),
            [] { TextureManager::current()->debug_print(std::cout); });

  add_hl();
  add_back(_("Back"));
}

void DebugMenu::menu_action(MenuItem& item) { g_config->save(); }
