...
std::unique_ptr<Sector> SectorParser::from_reader(Level& level,
...
std::unique_ptr<Sector> SectorParser::from_reader_old_format(
    Level& level, const ReaderMapping& reader, bool editable) {
  auto sector = std::make_unique<Sector>(level);
  BIND_SECTOR(*sector);
  SectorParser parser(*sector, editable);
...
std::unique_ptr<Sector> SectorParser::from_nothing(Level& level) {
  auto sector = std::make_unique<Sector>(level);
  BIND_SECTOR(*sector);
...
SectorParser::SectorParser(Sector& sector, bool editable)
    : m_sector(sector), m_editable(editable) {}

std::unique_ptr<GameObject> SectorParser::parse_object(
    const std::string& name_, const ReaderMapping& reader) {
...
void SectorParser::parse(const ReaderMapping& sector) {
...
void SectorParser::parse_old_format(const ReaderMapping& reader) {
  m_sector.set_name("main");

  float gravity;
  if (reader.get("gravity", gravity)) m_sector.set_gravity(gravity);
  ...
  */
  ...
  if (reader.get("interactive-tm", tiles) || reader.get("tilemap", tiles)) {
    auto* tileset =
        TileManager::current()->get_tileset(m_sector.get_level().get_tileset());
  ...
  if (m_sector.get_solid_tilemaps().empty()) {
    log_warning << "sector '" << m_sector.get_name()
...
void SectorParser::create_sector() {
  auto tileset =
      TileManager::current()->get_tileset(m_sector.get_level().get_tileset());
