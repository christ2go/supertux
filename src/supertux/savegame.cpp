...
void LevelsetState::store_level_state(const LevelState& in_state) {
...
LevelState LevelsetState::get_level_state(const std::string& filename) const {
  auto it = std::find_if(level_states.begin(), level_states.end(),
...
std::unique_ptr<Savegame> Savegame::from_file(const std::string& filename) {
...
bool Savegame::is_title_screen() const {
...
void Savegame::clear_state_table() {
  SquirrelVM& vm = SquirrelVirtualMachine::current()->get_vm();
...
std::vector<std::string> Savegame::get_worldmaps() {
  ...
  std::transform(worlds.begin(), worlds.end(), worlds.begin(),
                 physfsutil::realpath);

  return worlds;
}

WorldmapState Savegame::get_worldmap_state(const std::string& name) {
...
std::vector<std::string> Savegame::get_levelsets() {
...
LevelsetState Savegame::get_levelset_state(const std::string& basedir) {
...
void Savegame::set_levelset_state(const std::string& basedir,
