  ...
  void report_frame() {
    ...
    int dtime_us =
        static_cast<int>(std::chrono::duration_cast<std::chrono::microseconds>(
                             time_now - time_prev)
                             .count());
    if (dtime_us == 0) return;
    time_prev = time_now;

    acc_us += dtime_us;
    ++measurements_cnt;
    if (min_us > dtime_us) min_us = dtime_us;
    if (max_us < dtime_us) max_us = dtime_us;

    float expired_seconds = static_cast<float>(acc_us) / 1000000.0f;
    if (expired_seconds < 0.5f) return;
...
void ScreenManager::draw_fps(DrawingContext& context,
  ...
  context.color().draw_text(Resources::small_font, "FPS  min / avg / max", pos,
  ...
  pos.y += 15;
  context.color().draw_text(Resources::small_font, str3, pos, ALIGN_RIGHT,
                            LAYER_HUD);
  pos.x -= w3;
  context.color().draw_text(Resources::small_font, str2, pos, ALIGN_RIGHT,
                            LAYER_HUD);
  pos.x -= w2;
  context.color().draw_text(Resources::small_font, str1, pos, ALIGN_RIGHT,
                            LAYER_HUD);
}
...
void ScreenManager::draw(Compositor& compositor, FPS_Stats& fps_statistics) {
  ...
  if (g_config->show_fps) draw_fps(context, fps_statistics);
          ...
          case SDL_WINDOWEVENT_FOCUS_LOST:
            if (g_config->pause_on_focusloss) {
              if (session != nullptr && session->is_active() &&
                  !Level::current()->m_suppress_pause_menu) {
...
void ScreenManager::run() {
  ...
  while (!m_screen_stack.empty()) {
    ...
    int max_steps_per_frame =
        static_cast<int>(ceilf(seconds_per_frame / seconds_per_step));
