...
LevelIntro::LevelIntro(const Level& level,
    ...
    : m_level(level),
      ...
      m_power_sprite(SpriteManager::current()->create(
  ...
  if (m_player_status.bonus == FIRE_BONUS && g_config->christmas_mode) {
    ...
    m_player_sprite->set_action(m_player_status.get_bonus_prefix() +
                                "-walk-right");
  }
...
void LevelIntro::update(float dt_sec, const Controller& controller) {
  ...
  if (controller.pressed(Control::JUMP) ||
    ...
    ScreenManager::current()->pop_screen(
        std::make_unique<FadeToBlack>(FadeToBlack::FADEOUT, 0.1f));
  }
...
void LevelIntro::draw_stats_line(DrawingContext& context, int& py,
                                 const std::string& name,
                                 const std::string& stat, bool isPerfect) {
  std::stringstream ss;
  ss << name << ": " << stat;
  Color tcolor = isPerfect ? s_stat_perfect_color : s_stat_color;
  context.color().draw_center_text(Resources::normal_font, ss.str(),
...
void LevelIntro::draw(Compositor& compositor) {
  ...
  int py = static_cast<int>(static_cast<float>(context.get_height()) / 2.0f -
  ...
  {
    context.color().draw_center_text(Resources::normal_font, m_level.get_name(),
                                     Vector(0, static_cast<float>(py)),
                                     LAYER_FOREGROUND1, s_header_color);
    py += static_cast<int>(Resources::normal_font->get_height());
  }
  ...
  if ((!author.empty()) && (author != "SuperTux Team")) {
    ...
    context.color().draw_center_text(Resources::small_font, author_text,
                                     Vector(0, static_cast<float>(py)),
                                     LAYER_FOREGROUND1, s_author_color);
  ...
  {
    m_player_sprite->draw(context.color(),
                          Vector((static_cast<float>(context.get_width()) -
                                  m_player_sprite->get_current_hitbox_width()) /
                                     2,
                                 static_cast<float>(py) + m_player_sprite_py),
                          LAYER_FOREGROUND1);
  ...
  if (m_best_level_statistics) {
    context.color().draw_center_text(
        Resources::normal_font,
        std::string("- ") + _("Best Level Statistics") + std::string(" -"),
        Vector(0, static_cast<float>(py)), LAYER_FOREGROUND1, s_stat_hdr_color);
    ...
    draw_stats_line(
        ...
        Statistics::frags_to_string(m_best_level_statistics->m_badguys,
                                    stats.m_total_badguys),
        m_best_level_statistics->m_badguys >= stats.m_total_badguys);
    draw_stats_line(
        context, py, _("Secrets"),
        Statistics::secrets_to_string(m_best_level_statistics->m_secrets,
                                      stats.m_total_secrets),
        m_best_level_statistics->m_secrets >= stats.m_total_secrets);

    bool targetTimeBeaten =
        m_level.m_target_time == 0.0f ||
        (m_best_level_statistics->get_time() != 0.0f &&
         m_best_level_statistics->get_time() < m_level.m_target_time);
    draw_stats_line(
