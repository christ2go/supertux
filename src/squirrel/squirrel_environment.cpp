...
SquirrelEnvironment::SquirrelEnvironment(SquirrelVM& vm,
...
void SquirrelEnvironment::expose_self() {
  sq_pushroottable(m_vm.get_vm());
  m_vm.store_object(m_name.c_str(), m_table);
  sq_pop(m_vm.get_vm(), 1);
}

void SquirrelEnvironment::unexpose_self() {
  sq_pushroottable(m_vm.get_vm());
...
void SquirrelEnvironment::try_unexpose(GameObject& object) {
  auto script_object = dynamic_cast<ScriptInterface*>(&object);
  if (script_object != nullptr) {
...
void SquirrelEnvironment::unexpose(const std::string& name) {
  SQInteger oldtop = sq_gettop(m_vm.get_vm());
  sq_pushobject(m_vm.get_vm(), m_table);
  try {
...
void SquirrelEnvironment::run_script(const std::string& script,
...
void SquirrelEnvironment::garbage_collect() {
  m_scripts.erase(
...
void SquirrelEnvironment::wait_for_seconds(HSQUIRRELVM vm, float seconds) {
  m_scheduler->schedule_thread(vm, g_game_time + seconds);
}

void SquirrelEnvironment::update(float dt_sec) {
